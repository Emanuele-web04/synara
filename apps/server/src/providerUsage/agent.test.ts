...
describe("summarizeProviderUsageForAgent", () => {
  ...
  it("strips account identity from snapshots handed to agents", () => {
    const result = summarizeProviderUsageForAgent({
      provider: "grok",
      enabled: true,
      snapshot: snapshot({
        provider: "grok",
        detail: "Signed in as user@example.com",
        usageLines: [
          { label: "Account", value: "user@example.com" },
          { label: "Email", value: "user@example.com" },
          { label: "Note", value: "contact", subtitle: "user@example.com" },
    ...
    expect(result.availability).toBe("available");
    expect(result.snapshot?.detail).toBe("Signed in as [account]");
    expect(result.snapshot?.usageLines).toEqual([
      expect.objectContaining({ label: "Credits", value: "$12.34" }),
    ]);
