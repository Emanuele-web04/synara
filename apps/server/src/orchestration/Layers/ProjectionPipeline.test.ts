...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("does not double-count duplicate goal turn completion activities", () =>
    Effect.gen(function* () {
      const projectionPipeline = yield* OrchestrationProjectionPipeline;
      const eventStore = yield* OrchestrationEventStore;
      const sql = yield* SqlClient.SqlClient;
      const now = "2026-06-02T10:00:00.000Z";
      const completedAt = "2026-06-02T10:00:30.000Z";

      yield* eventStore.append({
        type: "thread.created",
        eventId: EventId.makeUnsafe("evt-goal-pipeline-thread"),
        aggregateKind: "thread",
        aggregateId: ThreadId.makeUnsafe("thread-goal-pipeline"),
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-goal-pipeline-thread"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-goal-pipeline-thread"),
        metadata: {},
        payload: {
          threadId: ThreadId.makeUnsafe("thread-goal-pipeline"),
          projectId: ProjectId.makeUnsafe("project-goal-pipeline"),
          title: "Goal Pipeline Thread",
          modelSelection: {
            provider: "codex",
            model: "gpt-5-codex",
          },
      ...
      yield* eventStore.append({
        ...
        eventId: EventId.makeUnsafe("evt-goal-pipeline-goal"),
        aggregateKind: "thread",
        aggregateId: ThreadId.makeUnsafe("thread-goal-pipeline"),
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-goal-pipeline-goal"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-goal-pipeline-goal"),
        metadata: {},
        payload: {
          threadId: ThreadId.makeUnsafe("thread-goal-pipeline"),
          goal: {
            id: "goal-pipeline",
            objective: "Ship persisted goal accounting",
            status: "active",
            tokenBudget: null,
            tokensUsed: 0,
      ...
      for (const sequence of [1, 2]) {
        yield* eventStore.append({
          ...
          eventId: EventId.makeUnsafe(`evt-goal-pipeline-activity-${sequence}`),
          aggregateKind: "thread",
          aggregateId: ThreadId.makeUnsafe("thread-goal-pipeline"),
          occurredAt: completedAt,
          commandId: CommandId.makeUnsafe(`cmd-goal-pipeline-activity-${sequence}`),
          causationEventId: null,
          correlationId: CommandId.makeUnsafe(`cmd-goal-pipeline-activity-${sequence}`),
          metadata: {},
          payload: {
            threadId: ThreadId.makeUnsafe("thread-goal-pipeline"),
            activity: {
              id: EventId.makeUnsafe("activity-goal-pipeline-completed"),
              tone: "info",
              kind: "turn.completed",
              summary: "Turn completed",
              payload: { usage: { inputTokens: 10, outputTokens: 5 } },
              turnId: TurnId.makeUnsafe("turn-goal-pipeline"),
      ...
      const rows = yield* sql<{ readonly goalJson: string }>`
        SELECT goal_json AS "goalJson"
        FROM projection_thread_goal
        WHERE thread_id = 'thread-goal-pipeline'
      `;
      const goal = JSON.parse(rows[0]!.goalJson) as {
        readonly turnCount: number;
        readonly tokensUsed: number;
        readonly usage: { readonly totalTokens: number };
      };
      assert.equal(goal.turnCount, 1);
      assert.equal(goal.tokensUsed, 15);
      assert.equal(goal.usage.totalTokens, 15);
    }),
  );
