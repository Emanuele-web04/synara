...
import { formatProviderDeliveryBlockDetail, PROVIDER_DELIVERY_BLOCK_SUMMARY } from "@synara/shared/providerDeliveryBlock";
...
describe("ProviderCommandReactor", () => {
  ...
  it.each([
    ...
    const harness = await createHarness({
      interruptTurn: () => Deferred.await(release).pipe(Effect.andThen(Effect.fail(
        new ProviderAdapterValidationError({
          provider: "codex", operation: "interruptTurn", issue: "Provider session closed.",
        }),
      ))),
    });
    const threadId = ThreadId.makeUnsafe("thread-1");
    ...
    const createdAt = new Date().toISOString();
    const session = {
      threadId, status: "running" as const, providerName: "codex" as const,
      runtimeMode: "approval-required" as const, activeTurnId: turnId,
      lastError: null, updatedAt: createdAt,
    };
    await Effect.runPromise(harness.engine.dispatch({
      type: "thread.session.set", commandId: CommandId.makeUnsafe("quit-race-session"),
      threadId,
      session: scenario.initiallyStopped ? { ...session, status: "stopped", activeTurnId: null } : session,
      createdAt,
    }));
    if (scenario.initiallyStopped) await Effect.runPromise(harness.engine.quiesce);
    const requested = await Effect.runPromise(harness.engine.dispatch({
      type: "thread.turn.interrupt",
      commandId: CommandId.makeUnsafe(scenario.intentionalQuit ? "quit-resume-interrupt:race:thread-1" : "manual-interrupt"),
      threadId, turnId, createdAt,
    }));
    if (!scenario.initiallyStopped) await waitFor(() => harness.interruptTurn.mock.calls.length === 1);
    await Effect.runPromise(harness.engine.quiesce);
    if (scenario.stopped) {
      await Effect.runPromise(harness.engine.dispatch({
        type: "thread.session.set", commandId: CommandId.makeUnsafe("quit-race-stopped"),
        threadId, session: { ...session, status: "stopped", activeTurnId: null }, createdAt,
      }));
    }
    await Effect.runPromise(Deferred.succeed(release, undefined));
    await harness.drain();
    const delivery = await Effect.runPromise(harness.deliveryRepository.getDelivery({
      consumerName: PROVIDER_COMMAND_REACTOR_CONSUMER, eventSequence: requested.sequence,
    }));
    expect(Option.getOrThrow(delivery).state).toBe("succeeded");
    const thread = await readHarnessThread(harness);
    expect(thread?.session?.activeTurnId).toBeNull();
    expect(thread?.activities.some((activity) => activity.kind === "provider.turn.interrupt.failed")).toBe(scenario.reportsFailure);
    expect(harness.sendTurn).not.toHaveBeenCalled();
    expect(harness.interruptTurn).toHaveBeenCalledTimes(scenario.initiallyStopped ? 0 : 1);
  });

  it.each([
    "proven quit", "unrelated session error", "missing stop", "manual interrupt",
    "ambiguous provider failure", "newer running session", "late stop", "failed stop",
    "turn start", "another blocker",
    ...
    const lastError = scenario === "ambiguous provider failure"
    ...
    const commandId = CommandId.makeUnsafe(scenario === "manual interrupt"
      ? "user-interrupt" : "quit-resume-interrupt:fixture:thread-1");
    const requested = await Effect.runPromise(harness.engine.dispatch(scenario === "turn start" ? {
      type: "thread.turn.start", commandId, threadId,
      message: { messageId: asMessageId("quit-start"), role: "user", text: "Do not replay", attachments: [] },
      runtimeMode: "approval-required", interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE, createdAt,
    } : {
      type: "thread.turn.interrupt", commandId, threadId, turnId: asTurnId("old-turn"), createdAt,
    }));
    const session = {
      threadId, status: "stopped" as const, providerName: "codex" as const,
    ...
    if (scenario !== "missing stop") {
      ...
      await Effect.runPromise(harness.engine.dispatch({
        type: "thread.session.set", commandId: CommandId.makeUnsafe("recorded-provider-stop"),
        threadId, session: { ...session, updatedAt: stoppedAt, lastError: scenario === "failed stop" ? "Exit unproven" : null },
        createdAt: stoppedAt,
      }));
    }
    if (scenario === "newer running session") {
      await Effect.runPromise(harness.engine.dispatch({
        type: "thread.session.set", commandId: CommandId.makeUnsafe("new-provider-session"),
        threadId, session: { ...session, status: "running", activeTurnId: asTurnId("new-turn") }, createdAt,
      }));
    }
    await Effect.runPromise(harness.deliveryRepository.claim({
      consumerName: PROVIDER_COMMAND_REACTOR_CONSUMER, eventSequence: requested.sequence,
      threadId, claimOwner: "old-process", claimedAt: createdAt, claimExpiresAt: createdAt,
    }));
    await Effect.runPromise(harness.deliveryRepository.markTerminalFailure({
      consumerName: PROVIDER_COMMAND_REACTOR_CONSUMER, eventSequence: requested.sequence,
      expectedClaimOwner: "old-process", state: "uncertain", error: lastError, updatedAt: failedAt,
    }));
    if (scenario === "another blocker") {
      const another = await Effect.runPromise(harness.engine.dispatch({
        type: "thread.task.stop", commandId: CommandId.makeUnsafe("other-stop"), threadId,
        taskId: "other-task", createdAt,
      }));
      await Effect.runPromise(harness.deliveryRepository.claim({
        consumerName: PROVIDER_COMMAND_REACTOR_CONSUMER, eventSequence: another.sequence,
        threadId, claimOwner: "old-process", claimedAt: createdAt, claimExpiresAt: createdAt,
      }));
      await Effect.runPromise(harness.deliveryRepository.markTerminalFailure({
        consumerName: PROVIDER_COMMAND_REACTOR_CONSUMER, eventSequence: another.sequence,
        expectedClaimOwner: "old-process", state: "uncertain", error: "Acceptance unknown", updatedAt: failedAt,
      }));
    }
    const skippedAt = "2026-09-07T14:00:03.000Z";
    await Effect.runPromise(harness.engine.dispatch({
      type: "thread.turn.start", commandId: CommandId.makeUnsafe("quit-resume:skipped"), threadId,
      message: { messageId: asMessageId("skipped"), role: "user", text: "Never replay this signup", attachments: [] },
      runtimeMode: "approval-required", interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE, createdAt: skippedAt,
    }));
    const sessionError = scenario === "unrelated session error"
      ? "An unrelated failure" : formatProviderDeliveryBlockDetail(lastError);
    await Effect.runPromise(harness.engine.dispatch({
      type: "thread.session.set", commandId: CommandId.makeUnsafe("visible-delivery-error"),
      threadId, session: { ...session, status: "error", lastError: sessionError, updatedAt: skippedAt }, createdAt: skippedAt,
    }));
    for (const event of await Effect.runPromise(Stream.runCollect(harness.engine.readEvents(0)))) {
      await Effect.runPromise(harness.deliveryRepository.advanceCursor({
        consumerName: PROVIDER_COMMAND_REACTOR_CONSUMER, eventSequence: event.sequence, updatedAt: skippedAt,
      }));
    }
    await harness.startReactor();
    await harness.drain();
    const recovered = ["proven quit", "unrelated session error", "another blocker"].includes(scenario);
    const delivery = await Effect.runPromise(harness.deliveryRepository.getDelivery({
      consumerName: PROVIDER_COMMAND_REACTOR_CONSUMER, eventSequence: requested.sequence,
    }));
    ...
    expect(thread?.activities.some((activity) => activity.summary === "Previous messages were not sent")).toBe(recovered);
    expect(harness.sendTurn).not.toHaveBeenCalled();
    expect(harness.interruptTurn).not.toHaveBeenCalled();
    ...
    if (recovered) expect(audits[0]).toMatchObject({ outcome: "abandon", note: expect.stringContaining("without replay") });
  });
