...
describe("ProviderCommandReactor", () => {
  ...
  it("never dispatches control when the ComputerService is unsupported (Stable)", async () => {
    const manager = new ComputerManager({ backend: new FakeComputerBackend() });
    const harness = await createHarness({
      computerService: {
        supported: false,
    ...
    try {
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.turn.start",
          commandId: CommandId.makeUnsafe("stable-computer-opt-in"),
          threadId,
          message: {
            messageId: asMessageId("stable-computer-message"),
            role: "user",
            text: "Continue",
            attachments: [],
          },
  ...
  it("passes a typed /computer-use through literally when unsupported", async () => {
    const manager = new ComputerManager({ backend: new FakeComputerBackend() });
    const harness = await createHarness({
      computerService: {
        supported: false,
    ...
    try {
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.turn.start",
          commandId: CommandId.makeUnsafe("stable-computer-slash"),
          threadId,
          message: {
            messageId: asMessageId("stable-computer-slash-message"),
            role: "user",
      ...
      await waitFor(() => harness.sendTurn.mock.calls.length === 1);
      const sent = harness.sendTurn.mock.calls[0]?.[0];
      // The invocation is not parsed or rewritten: the provider sees the text
      // as typed, and no durable intent is armed.
      expect(sent?.input).toContain("/computer-use open Calculator");
      expect(sent?.input).not.toContain("Use Synara Computer for this task.");
      expect(harness.startSession.mock.calls.at(-1)?.[1].enableComputerControl).toBe(false);
      expect(manager.canContinueChatControl(threadId)).toBe(false);
    } finally {
      await manager.dispose();
  ...
  it("starts a fork without computer control when unsupported", async () => {
    const manager = new ComputerManager({ backend: new FakeComputerBackend() });
    const parentId = ThreadId.makeUnsafe("thread-1");
    const childId = ThreadId.makeUnsafe("thread-fork-child");
    const harness = await createHarness({
    ...
    const createdAt = new Date().toISOString();
    try {
      // Even if a parent somehow armed intent, the unsupported gate keeps the
      // fork at off.
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.turn.start",
          commandId: CommandId.makeUnsafe("stable-fork-parent"),
          threadId: parentId,
          message: {
            messageId: asMessageId("stable-fork-parent-message"),
            role: "user",
            text: "Continue",
            attachments: [],
          },
      ...
      await waitFor(() => harness.sendTurn.mock.calls.length === 1);
      expect(manager.canContinueChatControl(parentId)).toBe(false);
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.fork.create",
          commandId: CommandId.makeUnsafe("stable-fork-create"),
          threadId: childId,
          sourceThreadId: parentId,
          projectId: asProjectId("project-1"),
          title: "Fork child",
      ...
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.turn.start",
          commandId: CommandId.makeUnsafe("stable-fork-child-turn"),
          threadId: childId,
          message: {
            messageId: asMessageId("stable-fork-child-message"),
            role: "user",
            text: "Continue",
            attachments: [],
          },
      ...
      expect(manager.canContinueChatControl(childId)).toBe(false);
    } finally {
      await manager.dispose();
    }
  });

  it("does not arm durable intent on a steer when unsupported", async () => {
    const manager = new ComputerManager({ backend: new FakeComputerBackend() });
    const harness = await createHarness({
      computerService: {
        supported: false,
    ...
    try {
      ...
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.session.set",
          commandId: CommandId.makeUnsafe("stable-steer-session"),
          threadId,
          session: {
            threadId,
            status: "running",
      ...
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.turn.start",
          commandId: CommandId.makeUnsafe("stable-steer-activation"),
          threadId,
          message: {
            messageId: asMessageId("stable-steer-message"),
            role: "user",
