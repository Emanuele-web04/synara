...
// FILE: ProviderRuntimeIngestion.state.ts
// Purpose: Per-session ingestion buffers — turn message-id sets, buffered assistant text,
//   and buffered proposed-plan text — with their append/take/clear lifecycle helpers.
// Layer: dependency-built once per ingestion factory via makeIngestionState().
// Exports: IngestionState, makeIngestionState.

export type IngestionState = ReturnType<typeof makeBufferHelpers>;

export const makeIngestionState = Effect.fnUntraced(function* () {
...
function makeBufferHelpers(caches: {
  turnMessageIdsByTurnKey: Cache.Cache<string, Set<MessageId>>;
  bufferedAssistantTextByMessageId: Cache.Cache<MessageId, string>;
  bufferedProposedPlanById: Cache.Cache<string, { text: string; createdAt: string }>;
}) {
  const { turnMessageIdsByTurnKey, bufferedAssistantTextByMessageId, bufferedProposedPlanById } =
    caches;
