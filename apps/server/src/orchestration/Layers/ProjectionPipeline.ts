...
const makeOrchestrationProjectionPipeline = Effect.gen(function* () {
  ...
  const goalAccountedActivityIds = new Set<string>();
    ...
    Effect.gen(function* () {
      switch (event.type) {
        ...
        case "thread.activity-appended": {
          ...
          const activityId = String(event.payload.activity.id);
          if (goalAccountedActivityIds.has(activityId)) {
            return;
          }
          ...
          if (
            existingActivities.some((activity) => activity.activityId === event.payload.activity.id)
          ) {
            return;
          }
          const existing = yield* projectionThreadGoalRepository.getByThreadId({
          ...
          goalAccountedActivityIds.add(activityId);
          return;
        }
