// Purpose: Pure model/provider/reasoning/context-window formatting and flattening helpers for OpenCode/Kilo.
...
import { asPositiveInteger } from "./OpenCodeAdapter.token.ts";
import type {
  OpenCodeInventoryProvider,
...
export function isOpenCodeManagedProvider(provider: OpenCodeInventoryProvider) {
...
export function compareOpenCodeModelDescriptors(
  left: OpenCodeModelDescriptor,
  right: OpenCodeModelDescriptor,
) {
  const leftProvider = left.upstreamProviderName?.trim() || left.upstreamProviderId?.trim() || "￿";
  const rightProvider =
...
export function readOpenCodeInventoryVariantValue(
  variantKey: string,
...
export function normalizeOpenCodeReasoningDescriptors(input: {
...
export function inferOpenCodeDefaultReasoningEffort(
  providerId: string,
...
export function resolveOpenCodeModelReasoningSupport(
...
export function numberToContextWindowValue(value: unknown): string | null {
  if (typeof value === "string") {
    const trimmed = value.trim();
...
export function resolveOpenCodeContextWindowSupport(
  model: OpenCodeInventoryProvider["models"][string] | undefined,
): {
...
export function toOpenCodeModelDescriptor(input: {
...
export function formatOpenCodeCliProviderName(providerId: string): string {
...
export function fallbackOpenCodeProviderName(providerId: string): string {
  return providerId
    .split(/[-_/]+/u)
    .filter((segment) => segment.length > 0)
    .map((segment) => segment.charAt(0).toUpperCase() + segment.slice(1))
