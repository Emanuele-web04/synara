...
describe("ComputerLeaseReactor", () => {
  it("publishes waiting and resumed activity without forwarding child activity", async () => {
    const manager = new ComputerManager({ backend: new FakeComputerBackend() });
    const activity = vi.spyOn(manager.cursorActivity, "setRuntime");
    try {
      await withReactor(
        {
          manager,
          events: [
            event("request.opened", OWNER),
            {
              ...event("user-input.requested", OWNER),
              providerRefs: { providerParentThreadId: "parent-provider-thread" },
            },
            event("request.resolved", OWNER),
          ],
        },
        async () => {
          await waitUntil(async () => activity.mock.calls.length === 2);
          expect(activity.mock.calls).toEqual([
            [OWNER, "Needs approval", false],
            [OWNER, "Thinking", true],
          ]);
        },
      );
  ...
  it("does not release the parent lease for a child terminal event", () => {
