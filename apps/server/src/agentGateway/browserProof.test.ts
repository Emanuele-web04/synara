...
describe("browser proof artifacts", () => {
  it("stores unique private PNGs, does not trust a thread id as a path, and serves them to chat", async () => {
    ...
    try {
      ...
      expect(await resolveAllowedLocalPreviewFile({ requestedPath: first, cwd: null }))
        .toMatchObject({ path: await realpath(first), fileName: path.basename(first) });
      await expect(saveBrowserProof("thread", Buffer.from("not an image").toString("base64"), root)).rejects.toThrow("Invalid proof image");
    } finally {
