...
describe("agent gateway browser tools", () => {
  it("loads the delegated E2E playbook on demand without touching the browser", async () => {
    const execute = vi.fn();
    const tool = makeAgentGatewayBrowserTools({ available: true, execute: execute as never })
      .find(tool => tool.definition.name === "synara_e2e_review")!;
    const result = await Effect.runPromise(tool.handler({}, context));
    const text = JSON.stringify(result);
    for (const requirement of ["explicitly requests an E2E", "provider-native subagent/Task", "One agent owns the shared embedded browser", "Wait for the child", "If native delegation is unavailable", "Do not capture exposed secrets", "untested flows"]) {
      expect(text).toContain(requirement);
    }
    expect(execute).not.toHaveBeenCalled();
  });
  it.each([45000, 60000])("reports actionable timeout bounds before dispatch (%s)", async (timeoutMs) => {
    const execute = vi.fn();
    const run = makeAgentGatewayBrowserTools({ available: true, execute: execute as never }).find(tool => tool.definition.name === "browser_run")!;
    const result = await Effect.runPromise(run.handler({ timeoutMs, code: "private-code" }, context));
    expect(result.isError).toBe(true);
    const content = result.content[0];
    expect(JSON.parse(content?.type === "text" ? content.text : "null")).toMatchObject({ error: {
      code: "BrowserInvalidTimeout", phase: "input", effectMayHaveCommitted: false,
  ...
  it("forwards a bounded Betterwright operation and preserves legacy text output", async () => {
    const execute = vi.fn(() => Effect.succeed({tabId: TAB_ID, value: {visible: "Signed in"}, serializedByteCount: 23}));
    ...
    const result = await Effect.runPromise(run.handler({code: "return await snapshot()"}, context));
    expect(result.isError).not.toBe(true);
    expect(execute).toHaveBeenCalledWith(expect.objectContaining({name: "browser_run", threadId: "thread-a", sessionKey: context.callerSessionKey}));
    expect(result.content[0]).toMatchObject({type: "text", text: expect.stringContaining("Untrusted browser data")});
  ...
  it.each(["short-visible-result", "synthetic-page-content ".repeat(500)])(
    ...
    async (visible) => {
      const output = { tabId: TAB_ID, value: { visible }, serializedByteCount: Buffer.byteLength(JSON.stringify({ visible })) };
      const execute = vi.fn(() => Effect.succeed(output));
      const run = makeAgentGatewayBrowserTools({ available: true, execute })
        .find(tool => tool.definition.name === "browser_run")!;
      const result = await Effect.runPromise(run.handler({ code: "return await snapshot()" }, {
        ...context, callerProvider: "codex",
      }));
      expect(result.structuredContent).toEqual(output);
      expect(result.content).toEqual([{ type: "text", text: expect.stringContaining("Untrusted browser data") }]);
  ...
  it("keeps error details visible to Codex result-selection fallback", async () => {
    const execute = vi.fn();
    const run = makeAgentGatewayBrowserTools({ available: true, execute: execute as never })
      .find(tool => tool.definition.name === "browser_run")!;
    const result = await Effect.runPromise(run.handler({ code: "return true", timeoutMs: 60000 }, {
      ...context, callerProvider: "codex",
    }));
  ...
  it("preserves native screenshot blocks without duplicating metadata or base64", async () => {
    ...
    const metadata = {
      tabId: TAB_ID, url: "https://example.test/", capturedAt: "2026-09-07T00:00:00.000Z",
      mode: "viewport", clipped: false, image,
    };
    const execute = vi.fn(() => Effect.succeed({ structuredContent: metadata, image: { ...image, data: "YWJj" } }));
    const screenshot = makeAgentGatewayBrowserTools({ available: true, execute })
      .find(tool => tool.definition.name === "browser_screenshot")!;
    const result = await Effect.runPromise(screenshot.handler({}, { ...context, callerProvider: "codex" }));
    expect(result.isError).not.toBe(true);
  ...
  it("keeps unavailable status structured for Codex without contacting the host", async () => {
    const execute = vi.fn();
    const status = makeAgentGatewayBrowserTools({ available: false, execute: execute as never })
      .find(tool => tool.definition.name === "browser_status")!;
    const result = await Effect.runPromise(status.handler({}, { ...context, callerProvider: "codex" }));
  ...
  it.each(["saved", "failed", "debug"])("exposes completion proof truthfully when persistence is %s", async (mode) => {
    const image = { mimeType: "image/png" as const, width: 1, height: 1, byteLength: 3 };
    const execute = vi.fn(() => Effect.succeed({
      structuredContent: { tabId: TAB_ID, url: "https://example.test/", capturedAt: "2026-09-07T00:00:00Z", mode: "viewport", clipped: false, image },
    ...
    const tool = makeAgentGatewayBrowserTools({ available: true, execute }, { saveProof })
      .find(tool => tool.definition.name === "browser_screenshot")!;
    const result = await Effect.runPromise(tool.handler({ kind: mode === "debug" ? "debug" : "proof" }, { ...context, callerProvider: "codex" }));
    expect(result.isError).not.toBe(true);
    ...
    if (mode === "saved") expect(result.structuredContent).toHaveProperty("artifactPath", "/private/generated_images/proof.png");
  ...
  it("rejects oversized batches and cross-call upload refs before dispatch", async () => {
    ...
    expect((await Effect.runPromise(run.handler({code: "x".repeat(16385)}, context))).isError).toBe(true);
    expect((await Effect.runPromise(upload.handler({target: {ref:"e1",snapshotId:TAB_ID},paths:["file.txt"]},context))).isError).toBe(true);
    expect(execute).not.toHaveBeenCalled();
    expect(normalizeGatewayBrowserArguments("browser_upload",{selector:"input[type=file]",files:["file.txt"]})).toEqual({target:{selector:"input[type=file]"},paths:["file.txt"]});
  });
