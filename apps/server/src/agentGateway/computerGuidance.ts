...
export const COMPUTER_HELP_SECTIONS = {
  ...
  menus:
    "On macOS, for app commands (new document/note/tab, save, find, preferences), try computer_invoke_menu with the exact menu path first. Use a screenshot + coordinate click only when the control is absent from both the element list and the menu bar. Menus activate the app: visible-use consent is required, even in computer_run; missing/disabled items refuse. list_apps lists apps/pids; set_window_frame moves windows (observe unconfirmed results, never replay); verify_state checks live predicates; zoom magnifies regions; get_accessibility_tree lists apps/windows (contents: get_state); get_cursor_position reads the pointer; kill_app force-quits and loses unsaved work (prefer authorized Quit).",
  hidden:
