...
/** the real profile, so tests break if it is renamed or removed */
...
describe("xcodeAppRoot", () => {
  it("climbs from a developer dir to the enclosing app bundle", () => {
    // the profile must cover the whole bundle — the helper mmaps frameworks from all over it
...
describe("sandboxedHelperCommand", () => {
  ...
  it.runIf(darwin)("wraps the helper in sandbox-exec with every parameter bound", async () => {
    ...
    // an unbound parameter silently matches nothing — every one the profile references must be passed
    for (const name of [
      "HELPER_BUNDLE",
      "USER_HOME",
      "CORESIM_HOME",
  ...
  it.runIf(darwin)("resolves DARWIN_TMP through symlinks", async () => {
    // $TMPDIR is /var/folders/... which really lives at /private/var/folders/... — Seatbelt matches the real path, so an unresolved parameter denies the frame socket
...
describe("describeSandboxSuspicion", () => {
  ...
  it("says nothing when the helper was not confined", () => {
    // blaming a sandbox never applied would send the reader the wrong way
