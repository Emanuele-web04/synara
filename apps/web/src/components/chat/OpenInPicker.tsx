...
interface OpenInPickerProps {
  // Editor config is optional: callers that already hold it (e.g. the chat header) pass it through, while standalone surfaces (file-preview headers) omit it and let the picker self-fetch.
  keybindings?: ResolvedKeybindingsConfig;
  availableEditors?: ReadonlyArray<EditorId>;
  openInTarget: string | null;
  // "responsive" hides the label until the `header-actions` container is wide enough; "always" for surfaces that don't establish one
  labelMode?: "responsive" | "always";
  // "split" (default) renders the bordered chat-header split button; "compact" renders quiet icon-only ghost buttons for dense per-row surfaces (e.g. the changed-file rows), where labelMode is ignored and the label stays sr-only.
  variant?: "split" | "compact";
  // Pins the primary "Open" action to a specific editor for this surface without mutating the shared preferred-editor setting. The PDF viewer uses this to default to the OS viewer (e.g. Preview) while still listing installed editors.
  defaultEditor?: EditorId;
  // Lets a file surface reuse the installed-editor menu while its main action stays in-app. Omitting this preserves the normal preferred-editor action.
  primaryAction?: OpenInPickerPrimaryAction;
  // Surface-specific actions appended after the shared installed-editor list. OpenInPicker owns the separator so callers cannot create malformed menus.
...
function OpenInPickerWithConfig(props: OpenInPickerProps) {
  // The query-owning wrapper mounts only for standalone surfaces. Rows that receive config from ChatView avoid both the subscription and a QueryClient dependency in isolated rendering/tests.
...
// Quiet square ghost button shared by both compact controls so the pair reads as one unit; `data-popup-open` keeps the menu trigger highlighted while its menu is up.
