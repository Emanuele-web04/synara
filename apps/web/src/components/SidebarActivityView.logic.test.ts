...
describe("sidebar subagent activity", () => {
  ...
  it("keeps a parent's spinner running without undoing explicit Activity settlement", () => {
    ...
    expect(isThreadRunningForActivity(rolledUpParent)).toBe(true);
    expect(buildActivityViewModel({ threads, pinnedThreadIdSet: new Set() })).toMatchObject({
      active: [],
      settled: [rolledUpParent],
    });
  ...
  it("stays working until the last child settles, then clears on completion or removal", () => {
    ...
    for (const remaining of [[done], []]) {
      ...
      expect(isThreadRunningForActivity(threads[0]!)).toBe(false);
    }
  });
  ...
  it("keeps a parent with a ready plan running until its child finishes", () => {
    const planParent = {
      ...parent,
    ...
    expect(isThreadRunningForActivity(threads[0]!)).toBe(true);
