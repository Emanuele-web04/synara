...
function useSafariAccessInfo() {
  ...
  useEffect(() => {
    ...
    void request.then((value) => { if (!disposed) setInfo(value); })
      .catch(() => { if (!disposed) setInfo({ supported: false }); });
...
export function SafariAccessSetupButton() {
  ...
  return <Button size="sm" variant="outline" onClick={() => window.dispatchEvent(new Event(OPEN_EVENT))}>
...
export function SafariAccessOnboarding({ children }: { children?: ReactNode }) {
  ...
  useEffect(() => {
    const show = () => { setStatus(null); setRevisit(true); };
    window.addEventListener(OPEN_EVENT, show);
    return () => { generation.current++; window.removeEventListener(OPEN_EVENT, show); };
  }, []);
  ...
  const run = async (action: "openSettings" | "revealApp") => {
    ...
    try {
      ...
      if (request === generation.current) setStatus("Could not open the system window. Open System Settings > Privacy & Security > Full Disk Access manually.");
    } finally {
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={(value) => { if (!value) close("later"); }}>
      <DialogPopup showCloseButton={false} initialFocus={sheetRef} className="max-w-[480px]">
        <div ref={sheetRef} tabIndex={-1} className="min-h-0 space-y-4 overflow-y-auto p-5 outline-none">
          <DialogHeader className="gap-2 p-0">
            <DialogTitle>Set up Safari import</DialogTitle>
            <DialogDescription>
              To import signed-in Safari sessions, allow Synara Full Disk Access in macOS.
              This is optional. Chats, coding tasks and signing in directly work without it.
            </DialogDescription>
          </DialogHeader>
          ...
          {info?.supported ? <div className="space-y-2 text-sm">
            <p>In System Settings &gt; Privacy &amp; Security &gt; Full Disk Access, enable <strong>{info.appName}</strong>.
              If it is missing, use the + button to add this app.</p>
            {info.appPath ? <>
              <p className="break-all text-xs text-muted-foreground">{info.appPath}</p>
              <Button size="sm" variant="ghost" disabled={busy} onClick={() => { void run("revealApp"); }}>Show app in Finder</Button>
            </> : <p className="text-xs text-muted-foreground">Use the installed Synara app, not a terminal or helper process.</p>}
            <p>Use your installed copy in Applications, not a copy on the installation disk image.
              After changing access, fully quit and reopen Synara.</p>
          </div> : null}
          <p className="text-xs text-muted-foreground">Access is checked only when you import. You can revisit this in Settings &gt; General &gt; Safari import setup.</p>
          {status ? <p role="status" className="text-sm text-muted-foreground">{status}</p> : null}
          <DialogFooter className="flex-wrap gap-2 p-0">
            <Button variant="ghost" onClick={() => close("later")}>Set up later</Button>
            <Button variant="outline" onClick={() => close("continued")}>Continue to Synara</Button>
            <Button disabled={busy} onClick={() => { void run("openSettings"); }}>Open System Settings</Button>
          </DialogFooter>
        </div>
      </DialogPopup>
    </Dialog>
