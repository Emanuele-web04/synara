...
export function ProviderUsageActivityCard({
  ...
  if (usage.length === 0) {
    return <SettingsEmptyState layout="status">No usage recorded in Synara yet.</SettingsEmptyState>;
  }
  ...
  const reportedTokens = usage.filter((entry) => entry.tokensReported);
  const tokenSummary =
    reportedTokens.length === 0
      ? "Not reported"
      : reportedTokens.length === usage.length
        ? formatCompact(totalTokens)
        : `${formatCompact(totalTokens)} partial`;
  const costSummary =
    reportedCosts.length === 0
      ? "Not reported"
      : reportedCosts.length === usage.length
            ...
            return (
              ...
              >
                ...
                <span className="shrink-0 text-right text-xs tabular-nums text-muted-foreground">
                  {entry.tokensReported ? formatCompact(entry.tokens) : "Not reported"} tokens ·{" "}
                  {formatNumber(entry.turnCount)} turns
                </span>
              </div>
            );
          })}
