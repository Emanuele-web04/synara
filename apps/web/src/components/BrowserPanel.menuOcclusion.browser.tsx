...
import { ThreadId, type BrowserSetPanelBoundsInput, type ThreadBrowserState } from "@synara/contracts";
...
import { Menu, MenuItem, MenuPopupBase, MenuSub, MenuSubPopup, MenuSubTrigger, MenuTrigger } from "./ui/menu";
...
const state: ThreadBrowserState = {
  threadId, version: 1, open: true, activeTabId: "synthetic-tab", lastError: null,
  tabs: [{
    id: "synthetic-tab", url: "https://example.test/fixture", title: "Synthetic page",
    lastCommittedUrl: "https://example.test/fixture", runtimeSurface: "native", status: "live",
    isLoading: false, canGoBack: false, canGoForward: false, faviconUrl: null, lastError: null,
  }],
};
...
function DockFixture({ onAdd }: { onAdd: (kind: string) => void }) {
  return (
    <div className="flex h-screen w-screen">
      <button className="flex-1" type="button">Outside menu</button>
      <RightDock
        state={{ open: true, activePaneId: "browser", panes: [{
          id: "browser", kind: "browser", threadId, diffTurnId: null, diffFilePath: null,
          filePath: null, pullRequestProjectId: null, pullRequestRepository: null,
          pullRequestNumber: null, pullRequestInitialTab: null,
        }] }}
        minWidth={320} defaultWidth="50vw" shouldAcceptWidth={() => true}
        addMenuKinds={["terminal", "browser", "explorer", "sidechat"]}
        onClosePane={() => {}} onCollapse={() => {}} onOpenChange={() => {}}
        onAddPane={onAdd}
        renderPane={() => <BrowserPanel mode="sidebar" threadId={threadId} onClosePanel={() => {}} />}
      />
    </div>
  );
}
...
function FloatingFixture({ showMenu = true }: { showMenu?: boolean }) {
  ...
  return (
    <>
      ...
      {showMenu && <div style={{ position: "fixed", left: 560, top: 170 }}>
        <Menu modal={false}>
          ...
          <MenuPopupBase align="end">
            ...
            <MenuSub>
              ...
              <MenuSubPopup><MenuItem onClick={() => setSelected(true)}>Explorer</MenuItem></MenuSubPopup>
            </MenuSub>
          </MenuPopupBase>
        </Menu>
      </div>}
...
beforeEach(() => {
  ...
  useBrowserStateStore.setState({ threadStatesByThreadId: { [threadId]: state }, recentHistoryByThreadId: {} });
});
...
describe("native browser menu occlusion", () => {
  it("retains a renderer opener while its native popup is active and restores the same webview", async () => {
    ...
    const openerState: ThreadBrowserState = {
      ...state, tabs: [{ ...state.tabs[0]!, runtimeSurface: "renderer" }],
    };
    api.browser.open.mockResolvedValue(openerState);
    useBrowserStateStore.setState({ threadStatesByThreadId: { [threadId]: openerState } });
    const mounted = await render(<QueryClientProvider client={new QueryClient()}><DockFixture onAdd={() => {}} /></QueryClientProvider>);
    ...
    useBrowserStateStore.getState().upsertThreadState({
      ...openerState, version: 2, activeTabId: "popup",
      tabs: [...openerState.tabs, {
        ...state.tabs[0]!, id: "popup", openerTabId: state.activeTabId!, title: "Synthetic sign-in",
      }],
    });
  ...
  it.each([1280, 800])("restores the same docked tab after every Add panel dismissal at %ipx", async (width) => {
    await page.viewport(width, 900);
    const onAdd = vi.fn();
    const mounted = await render(<QueryClientProvider client={new QueryClient()}><DockFixture onAdd={onAdd} /></QueryClientProvider>);
    ...
    for (const dismissal of ["escape", "outside", "trigger", "selection"]) {
      await trigger.click();
      await expect.element(page.getByRole("menuitem", { name: "Explorer", exact: true })).toBeVisible();
      ...
      if (dismissal === "outside") await page.getByRole("button", { name: "Outside menu" }).click();
      if (dismissal === "trigger") await trigger.click();
      if (dismissal === "selection") await page.getByRole("menuitem", { name: "Explorer", exact: true }).click();
  ...
  it("keeps floating native content hidden through submenu interaction and restores on unmount", async () => {
    ...
    const mounted = await render(<QueryClientProvider client={client}><FloatingFixture /></QueryClientProvider>);
    await vi.waitFor(() => expect(lastBounds()?.width).toBeGreaterThan(0));
    const original = lastBounds();
    ...
    await page.getByRole("button", { name: "Panel menu" }).click();
    await vi.waitFor(() => expect(lastBounds()).toBeNull());
    await mounted.rerender(<QueryClientProvider client={client}><FloatingFixture showMenu={false} /></QueryClientProvider>);
    await vi.waitFor(() => expect(lastBounds()).toEqual(original));
    expect(api.browser.open).toHaveBeenCalledOnce();
    expect(api.browser.detachWebview).not.toHaveBeenCalled();
    await mounted.unmount();
