// authoring: prepend newest-first; `version` must match apps/web/package.json exactly (the dialog only opens when the installed build has a curated entry); `date` renders verbatim; each feature: id (stable, unique per release), title, description, optional image (path from apps/web/public) + details
