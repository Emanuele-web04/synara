...
// module scope so the run-always cleanup can be a try/finally — the Compiler doesn't compile module functions
...
function EventRouter() {
  ...
  // Right-dock sidechat panes render a full ChatView for their embedded thread, so they need a detail lease exactly like split-view panes: without one the sidechat's snapshot never syncs and its transcript stays on the loading state.
  ...
  useEffect(() => {
    ...
    // single choke point: the reducer ignores events for threads the store dropped, so advancing fence/cursor first would vouch for events that never landed and a later resume would skip them forever — on a missing thread, drop bookkeeping and re-snapshot
    const applyFencedThreadEvent = (threadId: ThreadId, event: OrchestrationEvent): boolean => {
      ...
      // Any applied event — live or replayed — is fresh activity: drop the replay backoff so a subsequently lost event repairs at base cadence.
    ...
    const unsubThreadEvent = api.orchestration.onThreadEvent((item) => {
      ...
      if (latestThreadSequence === undefined) {
        ...
        if (
          ...
          // Arm even while buffered: the immediate reconcile below may return a premature session-set snapshot, and the fence must outlive it (#548).
      ...
      if (
        ...
        // Arm after the generic post-event schedule so the fast first-reconcile delay is not overwritten back to the slower cadence.
        armThreadProjectionTerminalFence(threadId, item.event.sequence);
      } else {
    ...
    const unsubThreadStreamFailure = onThreadStreamFailure((failure) => {
      ...
      // The stream is dead with retries and reconnects exhausted: forget its cursor so a future resubscribe requests a fresh snapshot, and surface the failure so the thread view stops posing as an empty conversation.
      clearThreadDetailResumeCursor(threadId);
      threadSnapshotSequenceById.delete(threadId);
      threadSnapshotRequestInFlight.delete(threadId);
      threadSnapshotRefreshPending.delete(threadId);
