...
export function buildThemeCssVariables(
  ...
  // Codex paints the sidebar from the PRIMARY surface, not the darker under-surface — mirror in light and dark
  ...
  // mirror Codex's cmdk shell: thin the dropdown-background token 5% in oklab over the backdrop blur
  ...
  const appVariables: Record<string, string> = {
    ...
    // frosted blur only on translucent shells (macOS) — on opaque shells it becomes a GPU layer rasterized blurry on fractional DPI
    ...
    // opaque settings page → nothing to frost; skip the blur and its compositing cost
    "--app-settings-backdrop-filter": "none",
    // sheer fill + light blur over the desktop; dark deepens toward black — keep in sync with index.css :root/.dark fallbacks
    "--app-sidebar-surface":
      material === "translucent"
        ? variant === "dark"
        ...
        : sidebarSurface,
    // always opaque so settings matches the chat surface regardless of window material
