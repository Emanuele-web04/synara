...
function serializeHunkHeader(hunk: FileDiffMetadata["hunks"][number]): string {
...
/** Reconstruct unified-diff text for one parsed file from the model (not the DOM). */
...
/** Serialize an entire renderable patch for clipboard writes (every file, every line). */
