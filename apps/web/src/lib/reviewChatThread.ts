...
const VISIBLE_SEND_PREWARM_WAIT_MS = 3_000;
const REVIEW_CHAT_STILL_WARMING_REASON =
  "Review chat is still warming. Try again in a moment.";
...
type VisibleSendPrewarmResolution =
  ...
  | { readonly status: "warming"; readonly reason: string }
  | null;

function waitForPrewarmVisibleSendBudget(
  promise: Promise<ReviewChatPrewarmResult>,
): Promise<ReviewChatPrewarmResult | { readonly status: "warming"; readonly reason: string }> {
  return new Promise((resolve) => {
    ...
    const timeout = globalThis.setTimeout(() => {
      ...
      settled = true;
      resolve({ status: "warming", reason: REVIEW_CHAT_STILL_WARMING_REASON });
...
export async function sendReviewChatQuestion(input: {
  ...
  if (prewarmResolution?.status === "warming") {
    return { status: "unavailable", reason: prewarmResolution.reason };
  }
  ...
  const readyResolution = sessionReady
    ? resolution
    : await prewarmReviewChatThread({
        payload: input.payload,
        modelSelection,
        bootstrapReviewContext: false,
        refreshMetadata: false,
        api,
      }).catch(
        (error): ReviewChatThreadResult => ({
          status: "unavailable",
          reason: reviewChatUnavailableReason(error),
        }),
      );
  if (readyResolution.status !== "ready") {
    return readyResolution;
  }
  const adoptedPrewarm =
    prewarmResolution?.status === "ready" && prewarmResolution.threadId === readyResolution.threadId;
  input.onThreadReady?.(
    readyResolution.threadId,
    adoptedPrewarm ? false : readyResolution.created,
  );
  const bootstrapKey = reviewChatBootstrapKey({ target, payload: input.payload });
  const threadBootstrapKey = `${bootstrapKey}\u001f${readyResolution.threadId}`;
  const thread = useStore
    .getState()
    .threads.find((candidate) => candidate.id === readyResolution.threadId);
  if (thread && shouldUpdateThreadModelBeforeReviewTurn({ thread, modelSelection })) {
    await api.orchestration.dispatchCommand(
      buildMetaUpdateCommand({
        threadId: readyResolution.threadId,
        modelSelection,
      }),
    );
  }
  ...
  const includeReviewContext =
    ...
    (readyResolution.created || shouldBootstrapReviewContext(thread));
  const dispatchMode = isReviewContextBootstrapRunning(thread) ? "steer" : undefined;
  void api.orchestration
    .subscribeThread({ threadId: readyResolution.threadId })
    .catch(() => undefined);
  const createdAt = new Date().toISOString();
  await api.orchestration.dispatchCommand(
    buildTurnStartCommand({
  ...
  return {
    status: "sent",
    threadId: readyResolution.threadId,
    created: readyResolution.created,
