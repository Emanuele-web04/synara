...
// large enough that the rAF loop needs many quantized commits to drain it — partially-revealed states stay observable between polls
...
describe("useSmoothStreamedText", () => {
  ...
  it("snaps to the full text the instant streaming ends", async () => {
    ...
    // No trailing typewriter once the agent is done: the flip to settled must return the complete text synchronously, mid-animation backlog and all.
  ...
  it("resets instantly on a non-append replacement while streaming", async () => {
    ...
    // A projection repair can rewrite the text in place; a non-append target must not be typewriter-animated from a stale prefix.
