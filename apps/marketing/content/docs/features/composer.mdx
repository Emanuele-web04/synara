...
**Add to Chat** opens a small note composer anchored to the selection: write an optional comment or dictate it with the microphone, then confirm to attach the quote to the current draft. The note travels with the quote as one annotation, so the agent sees exactly which passage your remark applies to. Confirming with an empty note attaches the quote on its own, and Escape or clicking away attaches nothing.
