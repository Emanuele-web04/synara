...
The Linux AppImage also requires FUSE 2 (`libfuse.so.2`) to mount. Many current
distributions ship only FUSE 3, in which case the system FUSE 2 package must be
installed first:
