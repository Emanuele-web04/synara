---
title: Core concepts
description: Understand Synara's projects, tasks, environments, provider sessions, turns, and review workflow.
---

Synara becomes much easier to use once its ownership model is clear: **each task owns one body of work**—its conversation, provider session, working environment, tool activity, and Git changes.
...
<DocsScreenshot
  lightSrc="/synara-ui-light.png"
  darkSrc="/synara-ui-dark.png"
  alt="Synara workspace composition showing coding-agent tasks, terminals, browser verification, diffs, and Git worktrees"
  width={3216}
  height={2090}
  provenance="derived"
  caption="A representative composition of the shared workspace. Individual panels and available controls depend on the task, provider, and current Synara version."
/>
...
A provider is the coding-agent runtime Synara operates, such as Claude Code, Codex, OpenCode, Cursor, or another supported integration.
...
Each task owns a provider session. Available models and controls can differ because Synara discovers capabilities from the installed runtime and account.

## Handoffs

A [provider handoff](/docs/workflows/handoffs) lets another supported provider continue the same task and working environment using the context Synara passes to it.
...
Synara's checkpoint and revert controls can help recover task work, but committed Git history remains the strongest boundary for important changes.
