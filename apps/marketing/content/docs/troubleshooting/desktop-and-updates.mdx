...
5. Re-download the current installer from the [official download page](/install), or on macOS run `brew reinstall --cask synara` when the official cask is installed.
...
5. If the updater still fails, download the newest installer from [trysynara.com/install](/install) or [GitHub Releases](https://github.com/Emanuele-web04/synara/releases). Homebrew installs may use `brew upgrade --cask synara` after `brew update`.
