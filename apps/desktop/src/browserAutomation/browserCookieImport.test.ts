...
const mocks = vi.hoisted(() => ({
  sources: vi.fn(), profiles: vi.fn(), sync: vi.fn(), closeBrowser: vi.fn(), closeConnection: vi.fn(), connect: vi.fn(),
}));
vi.mock("betterwright", () => ({
  listCookieSourceBrowsers: mocks.sources,
  listCookieSourceProfiles: mocks.profiles,
  NetworkPolicy: class {},
  BetterWright: class { syncCookies = mocks.sync; close = mocks.closeBrowser; },
}));
...
const input = { threadId: ThreadId.makeUnsafe("thread-1"), tabId: "tab-1", scope: "site", origin: "https://example.test", browser: "chrome", profile: "Default" } as const;
beforeEach(() => {
  vi.clearAllMocks();
  mocks.sources.mockResolvedValue([{ id: "chrome", name: "Chrome" }, { id: "firefox", name: "Firefox" }]);
  mocks.profiles.mockResolvedValue([{ id: "Default", name: "Default" }]);
  mocks.sync.mockResolvedValue({ ok: true, synced: 2, skipped: 1, cookieImportDomains: ["example.test"], warnings: [{ code: "unsupported", count: 1 }], cookie: "must-not-be-returned" });
  mocks.closeBrowser.mockResolvedValue(undefined);
  mocks.closeConnection.mockResolvedValue(undefined);
  mocks.connect.mockResolvedValue({ provider: { cdpUrl: "ws://127.0.0.1:1234/browser" }, close: mocks.closeConnection });
});

function fixture(rememberSessionImport = vi.fn(async (_domains: readonly string[]) => {})) {
  const contents = Object.assign(new EventEmitter(), { getURL: () => input.origin, session: { cookies: { flushStore: vi.fn(async () => {}) } } });
  const releaseHumanOperation = vi.fn();
  const waitForAgents = vi.fn(async () => {});
  const manager = { beginHumanBrowserOperation: vi.fn(() => releaseHumanOperation), getCookieImportRuntime: vi.fn(async () => ({ webContents: contents })) };
  return { importer: new BrowserCookieImport("/synthetic-home", manager as unknown as DesktopBrowserManager, waitForAgents, rememberSessionImport), contents, manager, releaseHumanOperation, waitForAgents, rememberSessionImport };
}
...
describe("human-only cookie import", () => {
  it("imports only the selected visible site with app-bound injection disabled and returns counts", async () => {
    ...
    expect(await importer.import(input)).toEqual({ ok: true, imported: 2, skipped: 1, warnings: [{ code: "unsupported", count: 1 }] });
    expect(mocks.sync).toHaveBeenCalledWith({ source: { browser: "chrome", profile: "Default" }, domains: ["example.test"], windowsAppBound: "disabled", timeoutMs: 30_000, cloudConsent: "cdp:127.0.0.1:1234" });
  ...
  it("rejects other origins, profile paths and unsupported sources before extraction", async () => {
    ...
    await expect(importer.import({ ...input, profile: "/private/arbitrary-path" })).rejects.toThrow();
    expect(() => Schema.decodeUnknownSync(BrowserCookieImportInput)({ ...input, browser: "unsupported" })).toThrow();
  ...
  it("does not report success or release human control before durable cookie writes finish", async () => {
    ...
    let flushed!: () => void;
    contents.session.cookies.flushStore.mockImplementation(() => new Promise<void>((resolve) => { flushed = resolve; }));
    let completed = false;
    const pending = importer.import(input).then((result) => { completed = true; return result; });
  ...
  it("requires session restoration checkpointing before reporting import success", async () => {
    const remember = vi.fn(async (_domains: readonly string[]) => { throw new Error("synthetic-checkpoint-failure"); });
    const { importer, releaseHumanOperation } = fixture(remember);
    await expect(importer.import(input)).resolves.toMatchObject({ ok: false, code: "persistence_failed" });
  ...
  it("holds human control until agents drain and import cleanup finishes", async () => {
    ...
    let drain!: () => void;
    waitForAgents.mockImplementation(() => new Promise<void>((resolve) => { drain = resolve; }));
    ...
    expect(mocks.closeBrowser).toHaveBeenCalledTimes(1);
    expect(releaseHumanOperation).toHaveBeenCalledTimes(1);
    expect(releaseHumanOperation.mock.invocationCallOrder[0]).toBeGreaterThan(mocks.closeBrowser.mock.invocationCallOrder[0]!);
  });
  ...
  it("rechecks the destination after draining and releases the lease on failure", async () => {
    ...
    waitForAgents.mockImplementation(async () => { contents.getURL = () => "https://other.test"; });
  ...
  it("requires explicit profile consent and permits all-sites import from a blank tab", async () => {
    ...
    const profile = { threadId: input.threadId, tabId: input.tabId, browser: input.browser, profile: input.profile, scope: "profile" };
    for (const confirmed of [undefined, false, "true"]) expect(() => Schema.decodeUnknownSync(BrowserCookieImportInput)({ ...profile, confirmed })).toThrow();
    contents.getURL = () => "about:blank";
    const request = Schema.decodeUnknownSync(BrowserCookieImportInput)({ ...profile, confirmed: true });
    expect((await importer.import(request)).ok).toBe(true);
    expect(mocks.sync).toHaveBeenCalledWith({ source: { browser: "chrome", profile: "Default" }, windowsAppBound: "disabled", timeoutMs: 30_000, cloudConsent: "cdp:127.0.0.1:1234" });
  ...
  it.each([
    [{ cookieReaderCode: "source_extraction_failed", cookiePermissionDenied: true }, "permission_denied"],
    ...
    [{}, "transfer_failed"],
  ])("returns only a fixed error category, never native diagnostic content", async (diagnostic, code) => {
    const { importer, releaseHumanOperation } = fixture();
    mocks.sync.mockResolvedValue({ ok: false, error: "synthetic-private-diagnostic", ...diagnostic });
