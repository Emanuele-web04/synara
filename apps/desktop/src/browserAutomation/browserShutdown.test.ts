...
describe("browser shutdown", () => {
  it("closes pages before awaiting host CDP drain, then clears vault keys", async () => {
    const order: string[] = [];
    let drained!: () => void;
    const host = new Promise<void>((resolve) => { drained = resolve; });
    await shutdownBrowserServices({
      revokeHost: () => { order.push("revoke"); return host; },
      closePages: () => { order.push("close-pages"); drained(); },
      stopCapture: async () => { await host; order.push("capture-stopped"); },
      clearKeys: () => { order.push("keys-cleared"); },
    });
  ...
  it("drains both services and clears keys even when capture cleanup fails", async () => {
    ...
    await expect(shutdownBrowserServices({ revokeHost: async () => {}, closePages: () => {}, stopCapture: async () => { throw new Error("synthetic-cleanup-error"); }, clearKeys })).rejects.toThrow("synthetic-cleanup-error");
  ...
  it("still stops capture and clears keys when closing a page throws synchronously", async () => {
    ...
    const clearKeys = vi.fn();
    await expect(shutdownBrowserServices({
      revokeHost: async () => { throw new Error("host-error"); },
      closePages: () => { throw new Error("page-error"); },
      stopCapture,
      clearKeys,
