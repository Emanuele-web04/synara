...
describe("source desktop launch", () => {
  it("launches normal macOS starts through LaunchServices without secrets in argv", () => {
    const { spawnProcess } = captureSourceDesktopSpawn(
      { SYNARA_HOME: "/tmp/isolated", SYNARA_AUTH_TOKEN: "synthetic-secret", ELECTRON_RUN_AS_NODE: "1" },
      { electronPath: "/runtime/Synara (Dev).app/Contents/MacOS/Electron", launchViaMacOS: true },
    );
    expect(spawnProcess).toHaveBeenCalledWith("/usr/bin/open", [
      "-W", "-n", "-a", "/runtime/Synara (Dev).app", "--args", "/workspace/apps/desktop/dist-electron/main.js",
    ], expect.objectContaining({ env: expect.objectContaining({ SYNARA_HOME: "/tmp/isolated", SYNARA_AUTH_TOKEN: "synthetic-secret" }) }));
  ...
  it.each([
    ...
    expect(() => captureSourceDesktopSpawn({}, { ...overrides, launchViaMacOS: true })).toThrow("macOS application bundle");
  });
