...
async function smoke() {
  ...
  if (window.isDestroyed() || unrelated.isDestroyed()) throw new Error("Worker closed a user-owned window.");
  ...
  if (!keyboard.blocked || keyboard.value !== "safe-controla") throw new Error("Clipboard boundary or modifier cleanup failed.");
  ...
  if (!focus || await window.webContents.executeJavaScript("document.querySelector('input').value !== ''")) throw new Error("Native guest focus isolation failed.");
  ...
  if (window.isDestroyed() || unrelated.isDestroyed()) throw new Error("Cancellation closed a window.");
  const report = { result, keyboard: "passed", nativeFocus: "passed", cancellation: "passed", unrelatedTarget: "untouched", hostWindows: "alive" };
  await writeFile(path.join(home, "result.json"), JSON.stringify(report, null, 2));
  console.log(JSON.stringify(report));
}

void app.whenReady().then(smoke).then(() => app.exit(0), (error) => {
  console.error(error);
  app.exit(1);
});
