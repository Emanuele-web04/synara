...
await writeFile(path.join(stage, "package.json"), JSON.stringify({ name: "synara-browser-smoke", version: "1.0.0", main: "main.cjs" }));
const staged = new Map();

async function packageRoot(name, from) {
  const resolve = createRequire(path.join(from, "package.json"));
  try { return path.dirname(resolve.resolve(`${name}/package.json`)); }
  catch {
    let current = path.dirname(resolve.resolve(name));
    for (;;) {
      try { if (JSON.parse(await readFile(path.join(current, "package.json"), "utf8")).name === name) return current; } catch {}
...
async function stagePackage(name, source) {
  ...
  if (staged.has(name)) {
    if (staged.get(name) !== source) throw new Error(`Smoke fixture cannot flatten two versions of ${name}.`);
    return;
  }
  {
    const target = path.join(stage, "node_modules", name);
    staged.set(name, source);
    await cp(source, target, { recursive: true, filter: (entry) => entry === source || !path.relative(source, entry).split(path.sep).includes("node_modules") });
    ...
    for (const dependency of Object.keys({ ...manifest.dependencies, ...manifest.optionalDependencies })) {
      let root;
      try { root = await packageRoot(dependency, source); }
      catch (error) { if (manifest.optionalDependencies?.[dependency]) continue; throw error; }
      await stagePackage(dependency, root);
    }
  }
}

for (const name of ["betterwright", "ws"]) await stagePackage(name, await packageRoot(name, desktop));
const archive = path.join(home, "app.asar");
await asar.createPackageWithOptions(stage, archive, { unpack: "**/*.node" });
const child = spawn(require("electron"), [archive], { stdio: "inherit", env: { ...process.env, SYNARA_SMOKE_HOME: path.join(home, "runtime") } });
child.on("error", (error) => { console.error(error.message); process.exitCode = 1; });
child.on("exit", (code) => { console.log(`ASAR smoke artifacts: ${home}`); process.exitCode = code ?? 1; });
