...
login wins. Direct credentials remain available for API-key-only instances
...
2. Log the Claude CLI into that account once, redirecting its home:
   ...
   ```sh
   HOME=~/claude-work-home claude auth login --claudeai
   ```
...
mark it as secret. Instance environment variables are scoped to that instance;
use a home without a usable Claude OAuth login when direct API-key auth should
win.
