...
function inventory(root) {
  ...
  function visit(directory) {
    for (const name of readdirSync(directory).sort()) {
      ...
      else if (stat.isFile()) files.push({ path: relative(root, path).replaceAll("\\", "/"), bytes: stat.size });
    }
  }
  visit(root);
  return files;
...
function group(files, key) {
  ...
  return [...totals].map(([name, bytes]) => ({ name, bytes })).sort((a, b) => b.bytes - a.bytes || a.name.localeCompare(b.name));
}
...
function readAsar(path) {
  ...
  try {
    ...
    if (readSync(fd, prefix, 0, prefix.length, 0) !== prefix.length) throw new Error(`Truncated ASAR: ${path}`);
    const length = prefix.readUInt32LE(12);
    if (length < 2 || length > 128 * 1024 * 1024) throw new Error(`Invalid ASAR header: ${path}`);
    const header = Buffer.alloc(length);
    if (readSync(fd, header, 0, length, 16) !== length) throw new Error(`Truncated ASAR header: ${path}`);
    const files = [];
    function visit(entries, parent = "") {
      for (const [name, entry] of Object.entries(entries)) {
        const child = parent ? `${parent}/${name}` : name;
        if (entry.files) visit(entry.files, child);
        else if (!entry.link) files.push({ path: child, bytes: entry.size, unpacked: entry.unpacked === true, sha256: entry.integrity?.hash ?? null });
      }
    }
    visit(JSON.parse(header.toString("utf8")).files);
    return { ...summarize(files), packages: group(files.filter((file) => file.path.startsWith("node_modules/")), (file) => file.split("/").slice(1, file.split("/")[1].startsWith("@") ? 3 : 2).join("/")), files };
  } finally {
    closeSync(fd);
  }
}
...
if (!stages[0]) throw new Error("No retained desktop build stage found; build with --keep-stage first.");
const stage = join(stages[0], "app");
const dist = join(stage, "dist");
const packagedDirectories = readdirSync(dist).filter((name) => lstatSync(join(dist, name)).isDirectory() && (name.startsWith("mac") || name.endsWith("-unpacked")));
if (packagedDirectories.length !== 1) throw new Error(`Expected one native packaged application, got ${packagedDirectories.join(", ")}`);
...
const artifacts = readdirSync(dist).filter((name) => /\.(dmg|zip|AppImage|exe)$/.test(name)).map((name) => ({ name, bytes: lstatSync(join(dist, name)).size, sha256: createHash("sha256").update(readFileSync(join(dist, name))).digest("hex") }));
...
const report = {
  ...
  signed: false,
  method: "Unsigned production packaging; installed size is regular-file byte sum without following symlinks; installer size is exact file length. ASAR inventory is reported separately, not added again.",
  artifacts,
...
console.log(JSON.stringify({ ...report, asar: { ...report.asar, files: undefined, packages: report.asar.packages.slice(0, 30) } }, null, 2));
