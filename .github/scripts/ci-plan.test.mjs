...
import { affectedPackages, fullPlan, planChanges, planForEvent, readGraph, workspacePaths } from "./ci-plan.mjs";
...
const scenarios = [
  ...
  ["Web", "apps/web/src/store.ts", "typecheck,unit,browser,build,windows", ["core", "web", "server 1/2", "server 2/2"]],
  ["Browser test", "apps/web/src/components/ChatView.browser.tsx", "typecheck,unit,browser,build,windows", ["core", "web", "server 1/2", "server 2/2"]],
  ["Desktop", "apps/desktop/src/main.ts", "typecheck,unit,build,windows", ["core"]],
  ["Server/CLI", "apps/server/src/http.ts", "typecheck,unit,build,windows", ["core", "server 1/2", "server 2/2"]],
  ["Windows/process runtime", "packages/shared/src/processRuntime.ts", all],
  ["Migration", "apps/server/src/persistence/Migrations.ts", "typecheck,unit,build,windows,migrations", ["core", "server 1/2", "server 2/2"]],
...
for (const [name, path, enabled, rows] of scenarios) {
  test(`${name}: only explicitly affected lanes run`, () => {
    ...
    if (rows) assert.deepEqual(plan.matrix.include.map((row) => row.pkg), rows);
  });
}

test("Unknown paths, package manifests and runtime documentation run validation", () => {
  for (const path of ["package.json", "patches/runtime.patch", "new-app/index.ts", "apps/web/package.json", ".github/scripts/ci-plan.mjs"]) {
    assert.equal(flags.every((flag) => planChanges([path], graph)[flag]), true, path);
  }
  ...
  assert.equal(flags.every((flag) => planChanges([], graph)[flag]), true);
});
...
test("Large and mixed changes do not truncate the affected change set", () => {
  ...
  assert.equal(flags.every((flag) => planChanges(paths, graph)[flag]), true);
...
function fixture(t) {
  ...
  write(root, "package.json", JSON.stringify({ workspaces: { packages: ["apps/*", "packages/*", "scripts"] } }));
  for (const [name, path] of Object.entries(workspacePaths)) {
    const dependencies = graph[name].filter((dependency) => !(name === "@synara/desktop" && dependency === "@synara/cli"));
    write(root, `${path}/package.json`, JSON.stringify({ name, devDependencies: Object.fromEntries(dependencies.map((name) => [name, "workspace:*"])) }));
  }
...
test("Main, non-merge checkouts and broken graphs fail open to full validation", (t) => {
  const root = fixture(t);
  for (const [event, ref] of [["push", "refs/heads/main"], ["pull_request", "refs/pull/1/head"], ["pull_request", "refs/pull/1/merge"]]) {
    assert.equal(flags.every((flag) => planForEvent(root, event, ref)[flag]), true);
  }
});
...
const laneFlags = { "static-typecheck": "typecheck", unit: "unit", browser: "browser", build: "build", windows_process: "windows", migration_lineage: "migrations" };
function needsFor(plan) {
  ...
  for (const [lane, flag] of Object.entries(laneFlags)) needs[lane] = { result: plan[flag] ? "success" : "skipped" };
  return needs;
}
function gateStatus(needs) {
  return spawnSync(process.execPath, ["-e", gate], { env: { ...process.env, NEEDS_JSON: JSON.stringify(needs) } }).status;
}
test("Gate accepts only successful required lanes and intentional skips", () => {
  for (const [, path] of scenarios) assert.equal(gateStatus(needsFor(planChanges([path], graph))), 0);
...
test("Workflow retains the gate name, native checks, partitions and frozen installs", () => {
  ...
  for (const flag of flags) assert.match(workflow, new RegExp(`needs.changes.outputs.${flag} == 'true'`));
  for (const name of ["backendShutdown.windows.integration.test.ts", "MigrationBackup.test.ts", "MigrationReplay.test.ts", "windowsProcessEffect.test.ts"]) assert.ok(workflow.includes(name));
  const setup = readFileSync(new URL("../actions/setup-workspace/action.yml", import.meta.url), "utf8");
