...
import ImageIO
import XCTest
@testable import DoTheThingKit

final class DoTheThingKitTests: XCTestCase {
    func testCLIRecognizesGlobalHelpAndVersionFlags() throws {
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["-h"]), .help(command: nil))
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["--help"]), .help(command: nil))
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["-v"]), .version)
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["--version"]), .version)
    }

    func testCLIRecognizesCommandSpecificHelp() throws {
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["help", "snapshot"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["snapshot", "--help"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["doctor", "-h"]), .help(command: "doctor"))
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["call", "--help"]), .help(command: "call"))
    }

    func testCLIRecognizesDoctorJSONOutput() throws {
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["doctor"]), .doctor(format: .text))
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["doctor", "--json"]), .doctor(format: .json))
    }

    func testCLIRecognizesSingleToolCallCommand() throws {
        XCTAssertEqual(
            try parseDoTheThingCLI(arguments: ["call", "list_apps"]),
            .call(.single(toolName: "list_apps", argumentsJSON: nil, argumentsFile: nil))
        )

        XCTAssertEqual(
            try parseDoTheThingCLI(arguments: ["call", "get_app_state", "--args", #"{"app":"TextEdit"}"#]),
    ...
    func testCLIRecognizesJSONSequenceCallCommand() throws {
        ...
        XCTAssertEqual(
            try parseDoTheThingCLI(arguments: ["call", "--calls", calls]),
            .call(.sequence(
                callsJSON: calls,
                callsFile: nil,
    ...
    func testCLIRecognizesJSONSequenceCallCommandWithCustomSleep() throws {
        ...
        XCTAssertEqual(
            try parseDoTheThingCLI(arguments: ["call", "--calls", calls, "--sleep", "0.5"]),
    ...
    func testCLIRecognizesTurnEndedNotifyPayload() throws {
        ...
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["turn-ended"]), .turnEnded(payload: nil))
        XCTAssertEqual(try parseDoTheThingCLI(arguments: ["turn-ended", payload]), .turnEnded(payload: payload))
        XCTAssertEqual(
            try parseDoTheThingCLI(arguments: ["turn-ended", "--previous-notify", #"["/bin/true"]"#, payload]),
            .turnEnded(payload: payload)
        )
    }

    func testCLIRequiresSnapshotArgument() {
        XCTAssertThrowsError(try parseDoTheThingCLI(arguments: ["snapshot"])) { error in
            XCTAssertEqual(
                error as? DoTheThingCLIError,
                DoTheThingCLIError(
    ...
    func testCLIRejectsMixedCallSequenceInputs() {
        XCTAssertThrowsError(try parseDoTheThingCLI(arguments: ["call", "list_apps", "--calls", "[]"])) { error in
            XCTAssertEqual(
                error as? DoTheThingCLIError,
                DoTheThingCLIError(
    ...
    func testCLIRejectsSleepForSingleToolCall() {
        XCTAssertThrowsError(try parseDoTheThingCLI(arguments: ["call", "list_apps", "--sleep", "0.5"])) { error in
            XCTAssertEqual(
                error as? DoTheThingCLIError,
                DoTheThingCLIError(
    ...
    func testCLIRejectsInvalidSequenceSleepValue() {
        XCTAssertThrowsError(try parseDoTheThingCLI(arguments: ["call", "--calls", "[]", "--sleep", "-1"])) { error in
            XCTAssertEqual(
                error as? DoTheThingCLIError,
                DoTheThingCLIError(
    ...
    func testCLIRejectsUnknownOption() {
        XCTAssertThrowsError(try parseDoTheThingCLI(arguments: ["--verbose"])) { error in
            XCTAssertEqual(
                error as? DoTheThingCLIError,
                DoTheThingCLIError(
    ...
    func testGeneralHelpListsCommandsAndGlobalFlags() {
        ...
        XCTAssertTrue(help.contains("dothething [command] [options]"))
    ...
    func testResolvedVersionFallsBackWhenBundleHasNoVersionMetadata() {
        XCTAssertEqual(resolvedDoTheThingVersion(bundle: Bundle(for: Self.self)), openComputerUseVersion)
    }
    ...
    func testToolDefinitionsIncludeFastSequenceTool() {
        XCTAssertTrue(ToolDefinitions.all.contains { $0.name == "run_sequence" })
    }

    func testReadToolArgumentsAcceptsJSONObject() throws {
        let arguments = try readDoTheThingToolArguments(
    ...
    func testElementIndexAcceptsNumericCallSequenceArgument() throws {
        let calls = try readDoTheThingCallSequence(
    ...
    func testReadToolArgumentsRejectsNonObject() {
        XCTAssertThrowsError(try readDoTheThingToolArguments(json: #"["TextEdit"]"#, file: nil)) { error in
            XCTAssertEqual(
                error as? DoTheThingCLIError,
                DoTheThingCLIError(message: "--args must be a JSON object", helpCommand: "call")
            )
        }
    }
    ...
    func testRunCallSequenceStopsAfterFirstToolError() throws {
        let output = try runDoTheThingCall(
            .sequence(
    ...
    func testMacOSAppAgentProxyDecisionKeepsNonAutomationCommandsLocal() {
        for command in [
            DoTheThingCLICommand.turnEnded(payload: nil),
            .help(command: nil),
            .version,
        ] {
    ...
    func testMacOSAppAgentSocketPathIsScopedToUserAndAppBundle() {
        let temporaryDirectory = URL(fileURLWithPath: "/tmp")
        let stableAppURL = URL(fileURLWithPath: "/Applications/Do The Thing.app")
        let devAppURL = URL(fileURLWithPath: "/Users/me/Build/Do The Thing.app")

        let stablePath = macOSAppAgentSocketPath(
            temporaryDirectory: temporaryDirectory,
            appBundleURL: stableAppURL,
            userID: 501
        )
        let sameStablePath = macOSAppAgentSocketPath(
            temporaryDirectory: temporaryDirectory,
            appBundleURL: stableAppURL,
            userID: 501
        )
        let devPath = macOSAppAgentSocketPath(
            temporaryDirectory: temporaryDirectory,
            appBundleURL: devAppURL,
            userID: 501
        )
        let otherUserPath = macOSAppAgentSocketPath(
            temporaryDirectory: temporaryDirectory,
            appBundleURL: stableAppURL,
            userID: 502
        )

        XCTAssertEqual(stablePath, sameStablePath)
        XCTAssertNotEqual(stablePath, devPath)
        XCTAssertNotEqual(stablePath, otherUserPath)
        XCTAssertTrue(stablePath.hasPrefix("/tmp/dothething-agent-501-"))
        XCTAssertTrue(stablePath.hasSuffix(".sock"))
    }
    ...
    func testPermissionStatusFlagsSettingsGrantedRuntimeDeniedAsRepairState() {
        let status = PermissionStatus(
            kind: .accessibility,
            runtimeGranted: false,
            settingsRecordGranted: true,
            settingsReadable: true
        )

        XCTAssertEqual(status.status, .grantedButRuntimeDenied)
        XCTAssertFalse(status.isReady)
    }

    func testPermissionStatusTreatsRuntimeGrantAsReadyEvenWhenSettingsUnreadable() {
        let status = PermissionStatus(
            kind: .screenRecording,
            runtimeGranted: true,
            settingsRecordGranted: nil,
    ...
    func testPermissionDiagnosticsSummaryIncludesRuntimeMismatchStatus() {
        let diagnostics = PermissionDiagnostics(permissionStatuses: [
            PermissionStatus(
                kind: .accessibility,
                runtimeGranted: false,
                settingsRecordGranted: true,
        ...
        XCTAssertEqual(diagnostics.missingPermissions, [.accessibility])
        XCTAssertTrue(diagnostics.summary.contains("accessibility=grantedButRuntimeDenied"))
        XCTAssertTrue(diagnostics.summary.contains("screenRecording=ready"))
    }

    func testPermissionDiagnosticsDoesNotInferRuntimeRefreshFromUnreadableTccStore() {
        let diagnostics = PermissionDiagnostics(permissionStatuses: [
            PermissionStatus(
                kind: .accessibility,
                runtimeGranted: false,
        ...
        XCTAssertFalse(diagnostics.needsRuntimePermissionRefresh(.accessibility))
        XCTAssertFalse(diagnostics.needsRuntimePermissionRefresh(.screenRecording))
        XCTAssertFalse(diagnostics.needsRuntimePermissionRefresh)
    }

    func testPermissionDiagnosticsDoesNotRecommendRuntimeRefreshForPlainMissingPermission() {
        let diagnostics = PermissionDiagnostics(permissionStatuses: [
            PermissionStatus(
                kind: .accessibility,
                runtimeGranted: false,
                settingsRecordGranted: false,
                settingsReadable: true
            ),
            PermissionStatus(
                kind: .screenRecording,
    ...
    func testPreferredPermissionAppBundleURLPrefersInstalledCopyOverTransientRunningCopy() {
        let installed = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/dothething/dist/Do The Thing.app")
        let running = URL(fileURLWithPath: "/Users/example/projects/dothething/dist/Do The Thing.app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/dothething-debug/dist/Do The Thing.app")
    ...
    func testPreferredPermissionAppBundleURLPrefersRunningDevelopmentCopy() {
        let installed = URL(fileURLWithPath: "/Applications/Do The Thing.app")
        let running = URL(fileURLWithPath: "/Users/example/projects/dothething/dist/Do The Thing (Dev).app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/dothething-debug/dist/Do The Thing (Dev).app")
    ...
    func testPreferredPermissionAppBundleURLCanPreferRunningReleaseCopyOverStaleInstalledCopy() {
        let staleInstalled = URL(fileURLWithPath: "/Users/example/projects/dothething/dist/npm/dothething/dist/Do The Thing.app")
        let running = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/dothething/dist/Do The Thing.app")
    ...
    func testPreferredInstalledAppBundleURLUsesFirstDiscoveredInstalledCopy() {
        let applications = URL(fileURLWithPath: "/Applications/Do The Thing.app")
        let npm = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/dothething/dist/Do The Thing.app")
        let duplicateApplications = URL(fileURLWithPath: "/Applications/Do The Thing.app")
    ...
    func testPermissionGrantedRequiresRuntimePreflightForCurrentProcess() {
        XCTAssertTrue(permissionGranted(persisted: false, runtime: true))
        XCTAssertTrue(permissionGranted(persisted: nil, runtime: true))
        XCTAssertFalse(permissionGranted(persisted: true, runtime: false))
    ...
    func testInitializeResponseContainsToolsCapability() throws {
        ...
        let response = server.handle(line: #"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26","clientInfo":{"name":"test","version":"0.1.52"},"capabilities":{}}}"#)
        XCTAssertNotNil(response)
        XCTAssertTrue(response!.contains(#""name":"dothething""#))
    ...
    func testInitializeResponseContainsComputerUseInstructions() throws {
        ...
        let response = try XCTUnwrap(
            server.handle(line: #"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26","clientInfo":{"name":"test","version":"0.1.52"},"capabilities":{}}}"#)
        )
    ...
    func testSnapshotActionResultOmitsFullAccessibilityTree() {
        let snapshot = makeSnapshot(
            treeLines: [
        ...
        XCTAssertTrue(rendered.contains("Action completed."))
        XCTAssertFalse(rendered.contains("\t0 standard window Sample Chat"))
        XCTAssertFalse(rendered.contains("\t2 button Send"))
        XCTAssertTrue(rendered.contains("Action results omit the full accessibility tree and screenshot for speed."))
        XCTAssertTrue(rendered.contains("Call get_app_state before using new element indices."))
    }

    func testSnapshotFullStateIncludesAccessibilityTree() {
        let snapshot = makeSnapshot(
            treeLines: ["\t2 button Send"],
            focusedSummary: nil
        )
        ...
        XCTAssertFalse(rendered.contains("Action completed."))
    }
    ...
    func testVisualCursorEnvFlagDefaultsToEnabled() {
        ...
        XCTAssertTrue(visualCursorEnabled(environment: ["DOTHETHING_VISUAL_CURSOR": "1"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["DOTHETHING_VISUAL_CURSOR": "0"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["DOTHETHING_VISUAL_CURSOR": "false"]))
    }

    func testInputFallbackDebugFlagDefaultsToDisabled() {
        XCTAssertFalse(inputFallbackDebugEnabled(environment: [:]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["DOTHETHING_DEBUG_INPUT_FALLBACKS": "1"]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["DOTHETHING_DEBUG_INPUT_FALLBACKS": "true"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["DOTHETHING_DEBUG_INPUT_FALLBACKS": "0"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["DOTHETHING_DEBUG_INPUT_FALLBACKS": "off"]))
    }

    func testGlobalPointerFallbackFlagDefaultsToDisabled() {
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: [:]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["DOTHETHING_ALLOW_GLOBAL_POINTER_FALLBACKS": "1"]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["DOTHETHING_ALLOW_GLOBAL_POINTER_FALLBACKS": "yes"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["DOTHETHING_ALLOW_GLOBAL_POINTER_FALLBACKS": "0"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["DOTHETHING_ALLOW_GLOBAL_POINTER_FALLBACKS": "false"]))
    }
    ...
    func testSoftwareCursorGlyphMetricsMatchRuntimeProceduralCalibration() {
        ...
        XCTAssertEqual(SoftwareCursorGlyphMetrics.sourceAssetName, "cursor-ai.svg")
    }

    func testSoftwareCursorGlyphThemeFollowsAppearance() throws {
        let darkAppearance = try XCTUnwrap(NSAppearance(named: .darkAqua))
        let lightAppearance = try XCTUnwrap(NSAppearance(named: .aqua))

        XCTAssertTrue(SoftwareCursorGlyphTheme.resolved(appearance: darkAppearance).isDark)
        XCTAssertFalse(SoftwareCursorGlyphTheme.resolved(appearance: lightAppearance).isDark)
    }
    ...
    func testOfficialCursorMotionTravelDurationScalesWithDistance() {
        ...
        let shortDuration = OfficialCursorMotionModel.calibratedTravelDuration(
            distance: 40,
            measurement: curvedMeasurement
        )
        let mediumDuration = OfficialCursorMotionModel.calibratedTravelDuration(
            distance: 140,
            measurement: curvedMeasurement
        )
        let longDuration = OfficialCursorMotionModel.calibratedTravelDuration(
            distance: 900,
            measurement: curvedMeasurement
        )

        XCTAssertLessThan(shortDuration, 0.40)
        XCTAssertLessThan(mediumDuration, 0.55)
        XCTAssertLessThan(longDuration, 1.0)
        XCTAssertLessThan(shortDuration, mediumDuration)
        XCTAssertLessThan(mediumDuration, longDuration)
        XCTAssertLessThan(longDuration, OfficialCursorMotionModel.closeEnoughTime)
    }
