...
enum OfficialCursorMotionModel {
    ...
    static func calibratedTravelDuration(distance: CGFloat, measurement: CursorMotionMeasurement) -> CGFloat {
        let clampedDistance = max(distance, minimumStepDistance)
        let baseDuration: CGFloat
        if clampedDistance < 80 {
            baseDuration = 0.28
        } else if clampedDistance < 400 {
            let progress = (clampedDistance - 80) / 320
            baseDuration = 0.28 + (progress * 0.34)
        } else {
            let progress = min((clampedDistance - 400) / 600, 1)
            baseDuration = 0.62 + (progress * 0.30)
        }

        let complexityBoost = min(measurement.angleChangeEnergy / 24, 0.12)
        let curvedBoost = measurement.length > 900 ? 0.08 : 0
        return min((baseDuration + complexityBoost + curvedBoost) / cursorSpeedMultiplier(), 1.0)
    }

    static func cursorSpeedMultiplier(environment: [String: String] = ProcessInfo.processInfo.environment) -> CGFloat {
        guard let rawValue = environment["DOTHETHING_CURSOR_SPEED"]?
