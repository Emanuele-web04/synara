...
public enum PermissionReadinessStatus: String, Sendable, Equatable {
    case ready
    case missing
    case grantedButRuntimeDenied
    case unknown
}

public struct PermissionStatus: Sendable, Equatable {
    public let kind: SystemPermissionKind
    public let runtimeGranted: Bool
    public let settingsRecordGranted: Bool?
    public let settingsReadable: Bool
    public let status: PermissionReadinessStatus

    public init(
        kind: SystemPermissionKind,
        runtimeGranted: Bool,
        settingsRecordGranted: Bool?,
        settingsReadable: Bool,
        status: PermissionReadinessStatus? = nil
    ) {
        self.kind = kind
        self.runtimeGranted = runtimeGranted
        self.settingsRecordGranted = settingsRecordGranted
        self.settingsReadable = settingsReadable
        self.status = status ?? Self.resolveStatus(
            runtimeGranted: runtimeGranted,
            settingsRecordGranted: settingsRecordGranted,
    ...
    var diagnosticsDictionary: [String: Any] {
        [
            "kind": kind.rawValue,
            "title": kind.title,
            ...
            "ready": isReady,
            "runtimeGranted": runtimeGranted,
            "settingsRecordGranted": settingsRecordGranted.map { $0 as Any } ?? NSNull(),
            "settingsReadable": settingsReadable,
        ]
    }
    ...
    private static func resolveStatus(
        ...
        if settingsRecordGranted == true {
            return .grantedButRuntimeDenied
        }
...
public struct PermissionDiagnostics: Sendable {
    public let permissionStatuses: [PermissionStatus]

    public init(accessibilityTrusted: Bool, screenCaptureGranted: Bool) {
        self.init(permissionStatuses: [
            PermissionStatus(
                kind: .accessibility,
                runtimeGranted: accessibilityTrusted,
                settingsRecordGranted: accessibilityTrusted,
                settingsReadable: true
            ),
            PermissionStatus(
                kind: .screenRecording,
                runtimeGranted: screenCaptureGranted,
                settingsRecordGranted: screenCaptureGranted,
                settingsReadable: true
            ),
        ])
    }

    public init(permissionStatuses: [PermissionStatus]) {
        self.permissionStatuses = SystemPermissionKind.allCases.compactMap { kind in
            permissionStatuses.first { $0.kind == kind }
        }
    }
    ...
    public static func current() -> PermissionDiagnostics {
        ...
        return PermissionDiagnostics(permissionStatuses: [
            PermissionStatus(
                kind: .accessibility,
                runtimeGranted: runtimeAccessibilityTrusted,
                settingsRecordGranted: persisted.accessibility,
                settingsReadable: persisted.accessibility != nil
            ),
            PermissionStatus(
                kind: .screenRecording,
                runtimeGranted: runtimeScreenCaptureGranted,
                settingsRecordGranted: persisted.screenRecording,
                settingsReadable: persisted.screenRecording != nil
            ),
        ])
    }

    public var summary: String {
        let parts = SystemPermissionKind.allCases.map { permission in
            "\(permission.rawValue)=\(permissionStatus(permission).status.rawValue)"
        }
        return "Permissions: \(parts.joined(separator: ", "))"
    }
    ...
    public func permissionStatus(_ permission: SystemPermissionKind) -> PermissionStatus {
        permissionStatuses.first { $0.kind == permission } ?? PermissionStatus(
            kind: permission,
    ...
    public func isGranted(_ permission: SystemPermissionKind) -> Bool {
        permissionStatus(permission).isReady
    }

    public func needsRuntimePermissionRefresh(_ permission: SystemPermissionKind) -> Bool {
        let status = permissionStatus(permission)
        guard !status.isReady else {
            return false
        }

        if status.status == .grantedButRuntimeDenied {
            return true
        }
    ...
    public var needsRuntimePermissionRefresh: Bool {
        SystemPermissionKind.allCases.contains { needsRuntimePermissionRefresh($0) }
    }

    public var allGranted: Bool {
        SystemPermissionKind.allCases.allSatisfy { permissionStatus($0).isReady }
    }

    public var repairMessage: String {
        if allGranted {
            return "Do The Thing permissions are ready. \(summary)"
        }

        let statuses = SystemPermissionKind.allCases.map { permissionStatus($0) }
        if needsRuntimePermissionRefresh {
            return "Do The Thing is enabled or partly enabled in macOS Settings, but the running helper has not picked up the latest permission state. Click Restart in the Do The Thing permission window, then retry this request. \(summary)"
        }

        if statuses.contains(where: { $0.status == .unknown }) {
            return "Do The Thing could not read the macOS permission record, and the running helper is not authorized yet. Open the Do The Thing permission window, confirm Accessibility and Screen Recording, then restart the helper if prompted. \(summary)"
        }

        return "Do The Thing needs macOS app permissions. I opened the Do The Thing permission window; approve the missing items once, then retry this request. \(summary)"
    }

    public var diagnosticsDictionary: [String: Any] {
        [
            "summary": summary,
            "allGranted": allGranted,
            "repairMessage": repairMessage,
            "needsRuntimePermissionRefresh": needsRuntimePermissionRefresh,
            "bundleIdentifier": Bundle.main.bundleIdentifier ?? "",
            "bundleURL": Bundle.main.bundleURL.standardizedFileURL.path,
            "permissionAppBundleURL": PermissionSupport.currentAppBundleURL()?.standardizedFileURL.path ?? NSNull(),
            "permissions": permissionStatuses.map { $0.diagnosticsDictionary },
        ]
    }

    public func diagnosticsJSONText() throws -> String {
        let data = try JSONSerialization.data(
            withJSONObject: diagnosticsDictionary,
            options: [.sortedKeys]
        )
...
public enum PermissionSupport {
    public static let bundleDisplayName = "Do The Thing"
    public static let bundleIdentifier = "com.synara.dothething"
    public static let developmentBundleDisplayName = "Do The Thing (Dev)"
    public static let developmentBundleIdentifier = "com.synara.dothething.dev"
    ...
    private static let appVariantInfoKey = "DoTheThingAppVariant"
    ...
    public static func isDoTheThingBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
    ...
    private static func preferredInstalledAppBundleURL() -> URL? {
        ...
        if let stableAppDir = env["SYNARA_DOTHETHING_STABLE_APP_DIR"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !stableAppDir.isEmpty
        {
            appendCandidate(
                URL(fileURLWithPath: stableAppDir, isDirectory: true)
                    .appendingPathComponent(releaseAppBundleName, isDirectory: true)
            )
        }

        // Always prefer the default Synara stable helper install before asking
        // LaunchServices. LaunchServices resolves the bundle identifier to any
        // registered copy (old builds, backups, deleted paths), which makes the
        // permission identity flap between copies and causes endless permission
        // onboarding loops.
        appendCandidate(
            URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
                .appendingPathComponent(".synara", isDirectory: true)
                .appendingPathComponent("dothething-app", isDirectory: true)
                .appendingPathComponent(releaseAppBundleName, isDirectory: true)
        )
        ...
        for prefix in homebrewPrefixes() {
            appendCandidate(prefix
                ...
                .appendingPathComponent("dothething", isDirectory: true)
                .appendingPathComponent(releaseAppBundleName, isDirectory: true)
            )
