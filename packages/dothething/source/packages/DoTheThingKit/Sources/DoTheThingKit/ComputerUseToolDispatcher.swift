...
public final class ComputerUseToolDispatcher {
    ...
    private func requireCallSequence(in arguments: [String: Any]) throws -> [DoTheThingCallSpec] {
        ...
        return try rawCalls.enumerated().map { index, item in
            ...
            return DoTheThingCallSpec(tool: tool, arguments: callArguments)
        }
    }

    private func runSequence(calls: [DoTheThingCallSpec], interCallDelay: TimeInterval) throws -> ToolCallResult {
...
public struct DoTheThingCallSpec {
...
public struct DoTheThingCallOutput {
...
public typealias DoTheThingSleepHandler = (TimeInterval) -> Void

public func runDoTheThingCall(
    _ invocation: DoTheThingCallInvocation,
    service: ComputerUseService = ComputerUseService(),
    sleepHandler: DoTheThingSleepHandler = { Thread.sleep(forTimeInterval: $0) }
) throws -> DoTheThingCallOutput {
    ...
    case let .sequence(callsJSON, callsFile, interCallDelay):
        ...
        return DoTheThingCallOutput(jsonObject: outputs, hasToolError: hasToolError)
    }
}

public func readDoTheThingToolArguments(
    json: String?,
    file: String?
) throws -> [String: Any] {
    guard let source = try readDoTheThingJSONSource(json: json, file: file) else {
        return [:]
    }

    let object = try decodeDoTheThingJSONObject(source)
    guard let arguments = object as? [String: Any] else {
        throw DoTheThingCLIError(message: "--args must be a JSON object", helpCommand: "call")
    }

    return arguments
}

public func readDoTheThingCallSequence(
    json: String?,
    file: String?
) throws -> [DoTheThingCallSpec] {
    guard let source = try readDoTheThingJSONSource(json: json, file: file) else {
        throw DoTheThingCLIError(message: "call sequence requires --calls or --calls-file", helpCommand: "call")
    }

    let object = try decodeDoTheThingJSONObject(source)
    guard let array = object as? [Any] else {
        throw DoTheThingCLIError(message: "--calls must be a JSON array", helpCommand: "call")
    }
    ...
    return try array.enumerated().map { index, item in
        ...
        return DoTheThingCallSpec(tool: tool, arguments: arguments)
    }
}

private func readDoTheThingJSONSource(json: String?, file: String?) throws -> String? {
    if json != nil, file != nil {
        throw DoTheThingCLIError(message: "Use either inline JSON or a JSON file, not both", helpCommand: "call")
    }
...
private func decodeDoTheThingJSONObject(_ source: String) throws -> Any {
    guard let data = source.data(using: .utf8) else {
        throw DoTheThingCLIError(message: "JSON input must be UTF-8 text", helpCommand: "call")
    }
