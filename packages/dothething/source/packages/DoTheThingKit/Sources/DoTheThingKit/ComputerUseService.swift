...
func inputFallbackDebugEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["DOTHETHING_DEBUG_INPUT_FALLBACKS"]?
...
func globalPointerFallbacksEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["DOTHETHING_ALLOW_GLOBAL_POINTER_FALLBACKS"]?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    else {
        return false
...
private enum SnapshotRefreshIntent {
    case fullState
    case actionResult
    case fastActionResult
}

public final class ComputerUseService {
    private var snapshotsByApp: [String: AppSnapshot] = [:]
    private var latestSnapshotEntry: SnapshotCacheEntry?
    private let snapshotFastReuseTTL: TimeInterval = 0.35
    ...
    public func getAppState(app query: String) throws -> ToolCallResult {
        snapshotResult(for: try refreshSnapshot(for: query, intent: .fullState), style: .fullState)
    }
    ...
    public func click(app query: String, elementIndex: String?, x: Double?, y: Double?, clickCount: Int, mouseButton: String) throws -> ToolCallResult {
        ...
        if snapshot.mode == .fixture {
            ...
            return snapshotResult(for: try refreshSnapshot(for: query, intent: .fastActionResult), style: .actionResult)
        }
    ...
    public func performSecondaryAction(app query: String, elementIndex: String, action: String) throws -> ToolCallResult {
        ...
        if snapshot.mode == .fixture {
            ...
            return snapshotResult(for: try refreshSnapshot(for: query, intent: .actionResult), style: .actionResult)
        }
    ...
    private func currentSnapshot(for query: String) throws -> AppSnapshot {
        ...
        return try refreshSnapshot(for: query, intent: .fullState)
    }

    @discardableResult
    private func refreshSnapshot(for query: String, intent: SnapshotRefreshIntent) throws -> AppSnapshot {
        let app = try AppDiscovery.resolve(query)
        let profile: ScreenshotEncodingProfile = intent == .fullState ? .fullState : .actionResult

        if intent == .fastActionResult,
           let entry = latestSnapshotEntry,
           Date().timeIntervalSince(entry.capturedAt) < snapshotFastReuseTTL,
           entry.snapshot.app.pid == app.pid,
           let refreshed = SnapshotBuilder.refreshScreenshot(from: entry.snapshot, profile: profile)
        {
            storeSnapshot(refreshed, query: query, app: app)
            return refreshed
        }

        let snapshot = try SnapshotBuilder.build(for: app, profile: profile)
    ...
    private func storeSnapshot(_ snapshot: AppSnapshot, query: String, app: RunningAppDescriptor) {
        latestSnapshotEntry = SnapshotCacheEntry(snapshot: snapshot, capturedAt: Date())
    ...
    private func debugInputFallback(tool: String, targetDescription: String, snapshot: AppSnapshot) {
        ...
        let appReference = snapshot.app.bundleIdentifier ?? snapshot.app.name
        fputs(
            "[dothething] global pointer fallback tool=\(tool) app=\(appReference) target=\(targetDescription)\n",
            stderr
        )
    }
    ...
    private func debugClickDecision(_ message: String) {
        ...
        fputs("[dothething] click decision \(message)\n", stderr)
    }
        ...
        do {
            ...
            guard globalPointerFallbacksEnabled(environment: ProcessInfo.processInfo.environment) else {
                throw ComputerUseError.message(
                    "click could not be handled through accessibility, and global pointer fallback is disabled. Set DOTHETHING_ALLOW_GLOBAL_POINTER_FALLBACKS=1 to allow physical-pointer fallback for this process."
                )
            }
        }
    }
    ...
    private func snapshotResult(for snapshot: AppSnapshot, style: SnapshotTextStyle) -> ToolCallResult {
        ...
        if style == .fullState, let screenshotPNGData = snapshot.screenshotPNGData {
