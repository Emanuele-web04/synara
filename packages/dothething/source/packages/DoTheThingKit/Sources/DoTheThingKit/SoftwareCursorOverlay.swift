...
func visualCursorEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["DOTHETHING_VISUAL_CURSOR"]?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() else {
        return true
    }
...
public func visualCursorObservationFileURL(environment: [String: String]) -> URL? {
    guard
        let rawPath = environment["DOTHETHING_VISUAL_CURSOR_OBSERVATION_FILE"]?
...
public let openComputerUseTurnEndedNotificationName = Notification.Name("com.synara.dothething.turn-ended")

public func postDoTheThingTurnEndedNotification() {
...
@MainActor
public func resetDoTheThingVisualCursor() {
