...
let screenshotActionResultMaxDimension: CGFloat = 960
...
private let shareableContentCacheTTL: TimeInterval = 3

enum ScreenshotEncodingProfile {
    case fullState
    case actionResult
    ...
    var maxDimension: CGFloat {
        switch self {
        case .fullState:
            return screenshotResultMaxDimension
        case .actionResult:
            return screenshotActionResultMaxDimension
        }
    }
}
...
private final class ShareableContentCache: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "dothething.shareable-content-cache")
    private var cachedContent: SCShareableContent?
    private var cachedAt = Date.distantPast

    func current() async throws -> SCShareableContent {
        let cached = queue.sync { () -> SCShareableContent? in
            if let cachedContent, Date().timeIntervalSince(cachedAt) < shareableContentCacheTTL {
...
public struct AppSnapshot {
    ...
    public func renderedText(style: SnapshotTextStyle) -> String {
        ...
        case .actionResult:
            lines.append("Action completed. Action results omit the full accessibility tree and screenshot for speed. Call get_app_state before using new element indices.")
        }
...
enum SnapshotBuilder {
    static func build(for app: RunningAppDescriptor, profile: ScreenshotEncodingProfile = .fullState) throws -> AppSnapshot {
        ...
        guard permissions.accessibilityTrusted else {
            throw ComputerUseError.permissionDenied(permissions.repairMessage)
        }
        ...
        var captureCandidate = WindowCapture.resolveCandidate(for: app.pid, titleHint: windowTitle)
        if captureCandidate == nil, recoverVisibleWindow(for: app, appElement: appElement, preferredWindow: rootWindow) {
            focusedApplication = copyElement(systemWide, attribute: kAXFocusedApplicationAttribute)
            if let recoveredWindow = preferredFocusedWindow(appElement: appElement, appPID: app.pid, focusedApplication: focusedApplication, systemWide: systemWide) {
                ...
                captureCandidate = WindowCapture.resolveCandidate(for: app.pid, titleHint: windowTitle)
            }
        }

        guard let captureCandidate else {
            throw ComputerUseError.stateUnavailable(computerUseNoWindowFoundMessage)
        }
    ...
    private static func buildAccessibilitySnapshot(
        ...
        windowTitle: String?,
        captureCandidate: WindowCaptureCandidate,
        focusedApplication: AXUIElement?,
        systemWide: AXUIElement,
        profile: ScreenshotEncodingProfile
    ) -> AppSnapshot {
        let windowBounds = captureCandidate.bounds
        ...
        final class CaptureImageBox: @unchecked Sendable {
            var image: CGImage?
        }

        let captureGroup = DispatchGroup()
        let captureBox = CaptureImageBox()
        captureGroup.enter()
        DispatchQueue.global(qos: .userInitiated).async {
            captureBox.image = WindowCapture.captureImage(
                windowID: captureCandidate.windowID,
                bounds: captureCandidate.bounds
            )
            captureGroup.leave()
        }
        ...
        let capturedImage = captureBox.image
        let windowCapture = WindowCapture(
            windowID: captureCandidate.windowID,
            layer: captureCandidate.layer,
            bounds: captureCandidate.bounds,
            image: capturedImage
        )
        let screenshotPNGData = windowCapture.pngDataIfAvailable(profile: profile)
    ...
    static func refreshScreenshot(from snapshot: AppSnapshot, profile: ScreenshotEncodingProfile) -> AppSnapshot? {
        guard let windowID = snapshot.targetWindowID, let bounds = snapshot.windowBounds else {
            return nil
        }

        let image = WindowCapture.captureImage(windowID: windowID, bounds: bounds)
        let screenshotPNGData = image.flatMap {
            boundedScreenshotPNGData(for: $0, maxDimension: profile.maxDimension)
        }
    ...
    private static func recoverVisibleWindow(for app: RunningAppDescriptor, appElement: AXUIElement, preferredWindow: AXUIElement?) -> Bool {
        ...
        if recovered {
            let deadline = Date().addingTimeInterval(windowVisibilityRecoveryDelay)
            while Date() < deadline {
                if WindowCapture.hasOnScreenWindow(for: app.pid) {
                    break
                }
...
private struct WindowCapture {
    ...
    static func hasOnScreenWindow(for pid: pid_t) -> Bool {
        resolveCandidate(for: pid, titleHint: nil) != nil
    }

    static func resolve(for pid: pid_t, titleHint: String?) -> WindowCapture? {
        guard let candidate = resolveCandidate(for: pid, titleHint: titleHint) else {
            return nil
        }

        let image = captureImage(windowID: candidate.windowID, bounds: candidate.bounds)
    ...
    static func resolveCandidate(for pid: pid_t, titleHint: String?) -> WindowCaptureCandidate? {
    ...
    static func captureImage(windowID: CGWindowID, bounds: CGRect) -> CGImage? {
        try? BlockingAsyncBridge.run(timeout: screenshotCaptureTimeout) {
            let shareableContent = try await ShareableContentCache.shared.current()
    ...
    func pngDataIfAvailable(profile: ScreenshotEncodingProfile = .fullState) -> Data? {
