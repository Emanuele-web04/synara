...
const appBundleName = "Do The Thing.app";
const appExecutableName = "DoTheThing";
const privatePackageName = "@t3tools/dothething";
const runtimeTargets = [
  ...
  },
  {
    ...
    kind: "binary",
    buildArch: "arm64",
    executablePath: ["dist", "linux", "arm64", "dothething"],
  },
  {
    os: "linux",
    cpu: "x64",
    kind: "binary",
    buildArch: "amd64",
    executablePath: ["dist", "linux", "amd64", "dothething"],
  },
  {
    os: "win32",
    cpu: "arm64",
    kind: "binary",
    buildArch: "arm64",
    executablePath: ["dist", "windows", "arm64", "dothething.exe"],
  },
  {
    os: "win32",
    cpu: "x64",
    kind: "binary",
    buildArch: "amd64",
    executablePath: ["dist", "windows", "amd64", "dothething.exe"],
  },
];
...
function ensureBuilt(configuration, arch) {
  run(path.join(repoRoot, "scripts", "build-dothething-app.sh"), [
    "--configuration",
    configuration,
    "--arch",
    arch,
  ...
  for (const buildArch of ["arm64", "amd64"]) {
    run(path.join(repoRoot, "scripts", "build-dothething-linux.sh"), [
      "--configuration",
      configuration,
      "--arch",
      buildArch,
    ]);
    run(path.join(repoRoot, "scripts", "build-dothething-windows.sh"), [
      "--configuration",
      configuration,
      "--arch",
      buildArch,
...
function runInstallCommand(scriptName, scriptArgs) {
  if (process.platform === "win32") {
    fail(\`\${command} currently requires a POSIX shell. Configure your MCP client with command "dothething" and args ["mcp"] on Windows.\`);
  }
...
Rebuild the private Synara package with:
  bun --cwd packages/dothething run build:macos\`);
  }
...
if (command === "help" && args[1] === "install-codex-plugin") {
  printInstallHelp("install-codex-plugin.sh", "dothething install-codex-plugin");
  process.exit(0);
}

if (command === "help" && args[1] === "install-codex-mcp") {
  printInstallHelp("install-codex-mcp.sh", "dothething install-codex-mcp");
  process.exit(0);
}

if (command === "help" && args[1] === "install-gemini-mcp") {
  printInstallHelp("install-gemini-mcp.sh", "dothething install-gemini-mcp [--scope project|user]");
  process.exit(0);
}

if (command === "help" && args[1] === "install-opencode-mcp") {
  printInstallHelp("install-opencode-mcp.sh", "dothething install-opencode-mcp");
  process.exit(0);
}

if (command === "help" && (args[1] === "install-claude-mcp" || args[1] === "install-clauce-mcp")) {
  printInstallHelp("install-claude-mcp.sh", "dothething install-claude-mcp");
  process.exit(0);
}
...
Synara's private desktop automation runtime for Do The Thing.

This package bundles the native Do The Thing runtime and exposes the local \`dothething\` / \`dothething-mcp\` commands used by Synara's MCP integration. It is intentionally marked \`private: true\` and is not a public npm package.
...
function renderMetaPackageJson(packageName, version) {
  return {
    ...
    version,
    private: true,
    description: "Synara's private desktop automation runtime for Do The Thing.",
    license: "MIT",
    keywords: ["computer-use", "codex", "mcp", "desktop-automation", "synara"],
    bin: {
      dothething: "bin/dothething",
      "dothething-mcp": "bin/dothething-mcp",
    },
    scripts: {
    ...
    files: [
      ...
      "bin/",
      "dist/Do The Thing.app/",
      "dist/linux/",
      "dist/windows/",
      "plugins/dothething/.codex-plugin/",
      "plugins/dothething/.mcp.json",
      "plugins/dothething/assets/",
      "plugins/dothething/scripts/",
...
function stagePrivatePackage(packageName, version, outDir) {
  ...
  cpSync(
    path.join(repoRoot, "plugins", "dothething"),
    path.join(stagedPackageRoot, "plugins", "dothething"),
    {
      recursive: true,
    },
  );
  ...
  writeExecutable(path.join(stagedPackageRoot, "bin", "dothething"), launcher);
  writeExecutable(path.join(stagedPackageRoot, "bin", "dothething-mcp"), launcher);
  writeFileSync(
