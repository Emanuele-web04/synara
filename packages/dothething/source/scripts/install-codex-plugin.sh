...
marketplace_name="dothething-local"
plugin_name="dothething"
plugin_source_root="${repo_root}/plugins/${plugin_name}"
plugin_manifest="${plugin_source_root}/.codex-plugin/plugin.json"
macos_build_script="${repo_root}/scripts/build-dothething-app.sh"
linux_build_script="${repo_root}/scripts/build-dothething-linux.sh"
windows_build_script="${repo_root}/scripts/build-dothething-windows.sh"
configuration="debug"
rebuild="false"
...
resolve_app_bundle() {
  ...
  if [[ "${configuration}" == "release" ]]; then
    candidates=("Do The Thing.app")
  else
    candidates=("Do The Thing (Dev).app" "Do The Thing.app")
  fi
...
resolve_native_binary() {
  ...
  case "${platform}" in
    linux)
      if [[ -n "${go_arch}" ]]; then
        candidates+=("${repo_root}/dist/linux/${go_arch}/dothething")
      fi
      ;;
    win32)
      if [[ -n "${go_arch}" ]]; then
        candidates+=("${repo_root}/dist/windows/${go_arch}/dothething.exe")
      fi
      ;;
  esac
...
case "${platform}" in
  darwin)
    ...
    app_binary="${payload_path:+${payload_path}/Contents/MacOS/DoTheThing}"
