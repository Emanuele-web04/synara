...
configuration="debug"
arch_mode="native"
codesign_mode="${DOTHETHING_CODESIGN_MODE:-auto}"
codesign_identity="${DOTHETHING_CODESIGN_IDENTITY:-}"
codesign_keychain="${DOTHETHING_CODESIGN_KEYCHAIN:-}"

usage() {
  cat <<'EOF'
Usage: ./scripts/build-dothething-app.sh [debug|release] [--configuration debug|release] [--arch native|arm64|x86_64|universal]

Examples:
  ./scripts/build-dothething-app.sh debug
  ./scripts/build-dothething-app.sh --configuration release --arch universal

Environment:
  DOTHETHING_CODESIGN_MODE=auto|identity|adhoc|none
  DOTHETHING_CODESIGN_IDENTITY="Developer ID Application: Example, Inc. (TEAMID)"
  DOTHETHING_CODESIGN_KEYCHAIN=/path/to/signing.keychain-db
EOF
}
...
if [[ "${codesign_mode}" != "auto" && "${codesign_mode}" != "identity" && "${codesign_mode}" != "adhoc" && "${codesign_mode}" != "none" ]]; then
  echo "Unsupported DOTHETHING_CODESIGN_MODE: ${codesign_mode}" >&2
  exit 1
fi

read_package_version() {
  python3 - "${repo_root}/plugins/dothething/.codex-plugin/plugin.json" <<'PY'
import json
import sys
...
build_binary() {
  ...
  swift build "${args[@]}" --product DoTheThing >&2
  printf '%s/DoTheThing\n' "${binary_dir}"
}
  ...
  case "${codesign_mode}" in
    ...
    identity)
      if [[ -z "${codesign_identity}" ]]; then
        echo "DOTHETHING_CODESIGN_IDENTITY is required when DOTHETHING_CODESIGN_MODE=identity" >&2
        exit 1
      fi
      printf '%s\n' "${codesign_identity}"
      return 0
...
codesign_app_bundle() {
  ...
  if ! identity="$(resolve_codesign_identity)"; then
    echo "Skipping codesign for ${app_path} (DOTHETHING_CODESIGN_MODE=none)" >&2
    return
  fi
...
bundle_version="${DOTHETHING_BUNDLE_VERSION:-$(git -C "${repo_root}" rev-list --count HEAD 2>/dev/null || echo 1)}"
release_app_bundle_name="Do The Thing.app"
development_app_bundle_name="Do The Thing (Dev).app"
legacy_app_bundle_name="DoTheThing.app"
bundle_icon_name="DoTheThing.icns"
icon_master_png="${repo_root}/assets/app-icons/dothething-1024.png"
...
bundle_display_name="Do The Thing"
bundle_identifier="com.synara.dothething"
app_variant="release"
app_bundle_name="${release_app_bundle_name}"

if [[ "${configuration}" != "release" ]]; then
  bundle_display_name="Do The Thing (Dev)"
  bundle_identifier="com.synara.dothething.dev"
  app_variant="dev"
...
case "${arch_mode}" in
  native)
    cp "$(build_binary "" "")" "${macos_dir}/DoTheThing"
    ;;
  arm64)
    cp "$(build_binary "arm64-apple-macosx14.0" ".build/arm64-${configuration}")" "${macos_dir}/DoTheThing"
    ;;
  x86_64)
    cp "$(build_binary "x86_64-apple-macosx14.0" ".build/x86_64-${configuration}")" "${macos_dir}/DoTheThing"
    ;;
  universal)
    ...
    lipo -create -output "${macos_dir}/DoTheThing" "${arm_binary}" "${x86_binary}"
    ;;
esac

chmod +x "${macos_dir}/DoTheThing"
...
icon_work_dir="$(mktemp -d "${TMPDIR:-/tmp}/dothething-icon.XXXXXX")"
cleanup() {
...
trap cleanup EXIT
iconset_dir="${icon_work_dir}/DoTheThing.iconset"
mkdir -p "${iconset_dir}"
