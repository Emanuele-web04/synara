...
const selectedAgents = new Set(
  (args.get("agents") ?? process.env.DOTHETHING_AGENT_AGENTS ?? "claude,codex")
    .split(",")
    .map((agent) => agent.trim())
    .filter(Boolean),
);
const json = args.get("json") === "true";
const command = args.get("command") ?? process.env.DOTHETHING_AGENT_COMMAND ?? "dothething";
const timeoutMs = Number(
  args.get("timeout-ms") ?? process.env.DOTHETHING_AGENT_TIMEOUT_MS ?? 120000,
);
const maxClaudeBudget =
  args.get("claude-budget-usd") ?? process.env.DOTHETHING_CLAUDE_BUDGET_USD ?? "2.00";
const hermesProvider = args.get("hermes-provider") ?? process.env.DOTHETHING_HERMES_PROVIDER;
const hermesModel = args.get("hermes-model") ?? process.env.DOTHETHING_HERMES_MODEL;
const hermesMaxTurns =
  args.get("hermes-max-turns") ?? process.env.DOTHETHING_HERMES_MAX_TURNS ?? "12";
...
function fixtureExecutablePath() {
  for (const candidate of [".build/release/DoTheThingFixture", ".build/debug/DoTheThingFixture"]) {
    if (existsSync(candidate)) return candidate;
  }

  const build = spawnSync("swift", ["build", "-c", "release", "--product", "DoTheThingFixture"], {
    cwd: process.cwd(),
    stdio: "inherit",
  });
  if (build.status !== 0) {
    throw new Error("Failed to build DoTheThingFixture.");
  }
  if (existsSync(".build/release/DoTheThingFixture")) return ".build/release/DoTheThingFixture";
  throw new Error("DoTheThingFixture executable was not found after build.");
}

function resetFixtureState() {
  rmSync(`${process.env.TMPDIR ?? "/tmp"}/dothething-fixture`, { recursive: true, force: true });
}
...
function callDoTheThing(tool, toolArgs = {}) {
...
function lineHasExactValue(text, id, value) {
  return text
    .split("\n")
    .some((line) => line.includes(`ID: ${id} `) && line.includes(` Value: ${value} Frame:`));
}
...
async function waitForFixtureReady(fixture) {
  ...
  while (Date.now() < deadline) {
    if (fixture?.fixtureExitCode !== null) {
      throw new Error(
        [
          `DoTheThingFixture exited before readiness with code ${fixture.fixtureExitCode}.`,
    ...
    try {
      const result = await callDoTheThing("get_app_state", { app: "DoTheThingFixture" });
  ...
  throw new Error(
    [
      "DoTheThingFixture did not become visible to dothething.",
...
async function validateFixtureState(expectedValue) {
  ...
  return {
    ok:
      lineHasExactValue(text, "fixture-input", expectedTextValue) &&
...
function scenarioAllowedTools() {
  return scenarioExpectedTools()
    .map((tool) => `mcp__dothething__${tool}`)
    .join(",");
}
...
function makePrompt(expectedValue) {
  if (scenario === "list-apps") {
    return [
      "Use the dothething MCP list_apps tool exactly once before answering.",
  ...
  return [
    "Use only dothething MCP tools.",
    "Call get_app_state for app DoTheThingFixture.",
    ...(scenario === "fixture-full"
      ? [
  ...
  return {
        ...
        JSON.stringify({
          mcpServers: {
            dothething: {
              ...
              command,
              args: ["mcp"],
              env: { DOTHETHING_VISUAL_CURSOR: "0" },
            },
          },
        }),
        "--permission-mode",
    ...
    codex: {
      ...
      args: [
        ...
        "--json",
        "-c",
        `mcp_servers.dothething.command=${JSON.stringify(command)}`,
        "-c",
        'mcp_servers.dothething.args=["mcp"]',
        prompt,
      ],
      validate: (result) =>
        result.stdout.includes("OPEN_CU_AGENT_OK") &&
        codexSawToolCalls(result.stdout, scenarioExpectedTools()),
    },
    hermes: {
      command: "hermes",
      args: [
...
for (const name of Object.keys(makeSpecs(""))) {
  ...
  const validated =
    result.ok && spec.validate(result) && (!usesFixture() || fixtureValidation?.ok === true);
  results.push({
    agent: name,
    processOk: result.ok,
    code: result.code,
...
if (json) {
  ...
  for (const result of results) {
    console.log(
      `- ${result.agent}: processOk=${result.processOk} validated=${result.validated} code=${result.code}`,
    );
    if (!result.validated) {
      if (result.stdoutTail)
        console.log(`  stdout: ${result.stdoutTail.replace(/\n/g, " ").slice(0, 500)}`);
      if (result.stderrTail)
        console.log(`  stderr: ${result.stderrTail.replace(/\n/g, " ").slice(0, 500)}`);
    }
  }
}
