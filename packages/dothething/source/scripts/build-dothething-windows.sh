...
print_help() {
  cat <<'EOF'
Usage:
  scripts/build-dothething-windows.sh [options]
...
version="$(node -e "console.log(require('./plugins/dothething/.codex-plugin/plugin.json').version)")"
module_dir="${repo_root}/apps/DoTheThingWindows"
...
(
  ...
  GOOS=windows GOARCH="${arch}" CGO_ENABLED=0 go build \
    ...
    -o "${output_dir}/dothething.exe" \
    .
)

echo "Built ${configuration} Windows runtime: ${output_dir}/dothething.exe"
