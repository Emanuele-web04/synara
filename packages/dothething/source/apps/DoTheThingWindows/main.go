...
func runPowerShell(request psRequest) (*psResponse, error) {
	...
	tempDir, err := os.MkdirTemp("", "dothething-windows-*")
	if err != nil {
		return nil, err
	}
...
func helpText(command string) string {
	...
	case "call":
		return "Usage:\n  dothething.exe call <tool> [--args '<json-object>']\n  dothething.exe call --calls '<json-array>'\n\nThe JSON array form keeps all calls in one process so element_index state can be reused.\n"
	case "snapshot":
		return "Usage:\n  dothething.exe snapshot <app>\n\nPrint the current Windows UI Automation snapshot for the target app.\n"
	default:
		return `Do The Thing for Windows

Usage:
  dothething.exe [command] [options]
