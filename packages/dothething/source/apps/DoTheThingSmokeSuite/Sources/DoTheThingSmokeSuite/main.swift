...
@main
enum DoTheThingSmokeSuite {
    static func main() throws {
        let productsDirectory = try locateProductsDirectory()
        let fixtureURL = productsDirectory.appendingPathComponent("DoTheThingFixture")
        let serverURL = productsDirectory.appendingPathComponent("DoTheThing")
        let appName = "DoTheThingFixture"
    ...
    private static func runCursorIdleSmoke(serverURL: URL, appName: String) throws {
        ...
        environment["DOTHETHING_VISUAL_CURSOR"] = "1"
        environment["DOTHETHING_VISUAL_CURSOR_OBSERVATION_FILE"] = observationURL.path
    ...
    private static func smokeServerEnvironment() -> [String: String] {
        ...
        environment["DOTHETHING_DISABLE_APP_AGENT_PROXY"] = "1"
        return environment
    }

    private static func smokeFixtureEnvironment() -> [String: String] {
        var environment = ProcessInfo.processInfo.environment
        environment["DOTHETHING_FIXTURE_HEADLESS"] = "1"
        return environment
    }
    ...
    private static func cursorObservationFileURL() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("dothething-smoke", isDirectory: true)
