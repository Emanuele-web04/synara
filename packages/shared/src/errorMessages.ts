// the server builds this invariant message and the client detects an Undo that raced another restore — one source so they can't drift
export const THREAD_NOT_ARCHIVED_INVARIANT_MARKER = "is not archived for command";

// a duplicate approval can race the authoritative update — the server rejects it while the client treats this marker as "refresh the settled request"
