...
/** Column + attention pills derived in one pass (one reason/heartbeat computation). */
...
/**
 * Single-pass card derivation for board renders: computes the awaiting-you
 * reason once and reuses it for both the column and the attention pills.
 * Behaviorally identical to calling `deriveKanbanColumnV2` (with `now`) and
 * `deriveKanbanAttention` separately, without re-deriving liveness per call.
 * `now` is required — a card view without a clock cannot classify Awaiting you.
 */
export function deriveKanbanCardView(
  t: KanbanThreadDerivationInput,
  opts: { now: number; needsReview?: boolean },
): KanbanCardView {
  const reason = deriveKanbanAwaitingYouReason(t, opts);
  const live = hasKanbanLiveWork(t);
  const column: KanbanColumnV2Key = reason
    ? "awaitingYou"
    : live
      ? "inProgress"
      : !t.latestTurn
        ? "draft"
        : "done";
  const attention: KanbanAttentionFlag[] = [];
  if (reason === "pending-approval") {
    attention.push("awaiting-approval");
  } else if (reason === "pending-input") {
    attention.push("awaiting-input");
  } else if (reason === "failed") {
    attention.push("failed");
  } else if (reason === "stuck") {
    attention.push("stuck");
  }
  // Same warn-stale rule as `deriveKanbanAttention`: the pill only adds "stuck"
  // while the card is still live and the reason flag does not carry it already.
  if (live && isWarnStuck(t, opts) && reason !== "stuck") {
    attention.push("stuck");
  }
  if (opts.needsReview === true) {
    attention.push("needs-review");
  }
  return { column, attention };
}
