...
describe("ProviderCommandReactor", () => {
  ...
  it("removes provider startup from a warmed review chat visible send", async () => {
    ...
    harness.startSession.mockImplementation((_: unknown, input: unknown) => {
      ...
      const threadId =
        ...
        typeof input.threadId === "string"
          ? ThreadId.makeUnsafe(input.threadId)
          : ThreadId.makeUnsafe("thread-cost");
      return Effect.succeed({
        ...
        model: "gpt-5-codex",
        threadId,
        resumeCursor: { opaque: "resume-cost" },
        createdAt: now,
        updatedAt: now,
  ...
  it("lets a visible review chat turn join an in-flight prewarm session ensure", async () => {
    ...
    harness.startSession.mockImplementationOnce((_: unknown, _input: unknown) =>
      Effect.promise(
        () =>
          new Promise<ProviderSession>((resolve) => {
            releaseSessionStart = () => {
              resolve({
                ...
                model: "gpt-5-codex",
                threadId: reviewThreadId,
                resumeCursor: { opaque: "resume-join-prewarm" },
                createdAt: now,
                updatedAt: now,
              });
            };
