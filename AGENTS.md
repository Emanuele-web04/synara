...
- UI text must follow the font size the user chose in Settings. Size it with the `--app-font-size-ui*` variables from [useAppTypography.ts](apps/web/src/hooks/useAppTypography.ts), for example `text-[length:var(--app-font-size-ui,12px)]` for body copy and `--app-font-size-ui-sm` for secondary text, and inherit the UI font family. Do not use fixed Tailwind sizes such as `text-sm`, `text-xs`, or `text-[11px]` for labels, descriptions, list rows, inputs, or button text. Only titles and headings may use a fixed size.
